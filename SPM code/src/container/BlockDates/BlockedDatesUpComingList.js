...
@observer
export default class BlockedDatesUpComingList extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      refreshing: false,
  ...
  componentWillMount() {
    this.setState({ propertyID: this.props.propertyID, propertyData: this.props.propertyData, refresh: this.props.refresh, loading: true });
    ...
    PropertyStore.getPropertyBlockedDates(this.props.propertyID, function (resObj) {
  ...
  _onRefresh = () => {
    ...
    PropertyStore.getPropertyBlockedDates(this.state.propertyID, function (resObj) {
      clearTimeout(isLoading)
      _this.setState({ loading: false });
      if (!PropertyStore.internet_connection) {
        _this.props.navigation.navigate('InformationScreen')
  ...
  render() {
    ...
    if (this.state.loading) {
    ...
    } else if (PropertyStore.BlockedDatesUpComingList.length <= 0) {
      ...
      userData=
        <FlatList
          ...
          data={PropertyStore.BlockedDatesUpComingList}
