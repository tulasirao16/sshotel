...
import { ActivityIndicator, View, Text, TouchableWithoutFeedback, TouchableHighlight, StyleSheet, BackHandler, Dimensions, StatusBar, Animated, TouchableOpacity, Platform, ScrollView, RefreshControl, FlatList } from 'react-native';
import { observer, inject } from 'mobx-react';
import { Container, Header, Content, List, ListItem, Card, CardItem, Body, Switch, Icon, Button } from 'native-base';
import styles from '../ReviewRatings/css/ReviewRatingsListCss';
...
@inject(['SPStore'])
@observer
export default class HomeScreenReviewRatings extends React.Component {
  ...
  componentWillMount() {
    ...
    SPStore.getAllReviewRatings('1', '', byDateType, function (resObj) {
    ...
    if (reviewStatus == 'Active') {
      SPStore.getstatusRatings(reviewId, 'Inactive', function (resObj) {
        ...
        if (resObj.statusCode == '0000') {
          SPStore.getAllReviewRatings('1', '', byDateType, function (resObj) {
            if (resObj.statusCode == '0000') {
              _this.setState({ statusCode: resObj.statusCode, allRatings: resObj.statusResult.myreviews })
            } else {
              _this.setState({ statusCode: resObj.statusCode, allRatings: [] })
            }
          })
        }
      })
  ...
  handleSearchChange = (searchString) => {
    ...
    const byDateType = navigation.state.params.byDateType;
    let _this = this;
    SPStore.getAllReviewRatings(1, searchString, byDateType, function (resObj) {
      if (resObj.statusCode == '0000') {
        _this.setState({ statusCode: resObj.statusCode, allRatings: resObj.statusResult.myreviews })
      } else {
        _this.setState({ statusCode: resObj.statusCode, allRatings: [] })
  ...
  handleSearchBackClick = (searchString) => {
    ...
    let _this = this;
    let isLoading = setTimeout(function () {
      _this.setState({ loading: false, reload: true, reloadFunction: 'handleSearchBackClick', param: searchString });
    }, 10000);
    SPStore.getAllReviewRatings(1, searchString, byDateType, function (resObj) {
      clearTimeout(isLoading)
      if (resObj.statusCode == '0000') {
        _this.setState({ loading: false, allRatings: resObj.statusResult.myreviews, refreshing: false })
      } else {
        _this.setState({ loading: false, allRatings: [], refreshing: false })
      }
    });
    this.searchBar.hide()
  }
  _onRefresh = (searchString) => {
    ...
    let _this = this;
    let isLoading = setTimeout(function () {
      _this.setState({ loading: false, reload: true, reloadFunction: '_onRefresh', param: searchString });
    }, 10000);
    SPStore.getAllReviewRatings(1, searchString, byDateType, function (resObj) {
      clearTimeout(isLoading)
      if (resObj.statusCode == '0000') {
  ...
  handleReload = () => {
    switch (this.state.reloadFunction) {
      ...
      case '_onRefresh':
        this.setState({ reload: false, reloadFunction: '' });
        this._onRefresh(this.state.param)
        break;
          ...
          <LinearGradient colors={['#01a4a2', '#025d8c']} start={[0.0, 0.5]} end={[1.0, 0.5]} locations={[0.0, 1.0]} style={styles.linearStyles}>
            ...
            <View style={styles.headerMainView} >
              ...
              <View style={styles.headerBody} >
                <Text style={styles.headerTitleStyle}>{i18n.t('lanTitleRatings&ReviewsList')} </Text>
              </View>
