...
export default class SPPropertyImagesEdit extends React.Component {
  constructor (props) {
    super(props)
    this.state = {
  ...
  handleBack = () => {
    this.props.imageFunction(this.state.photos, this.state.newFiles, this.state.video, this.state.videoFiles)
  }
  handleSelect = (item, i) => {
    let _this = this
    ...
    async.series([
      ...
      if (item._id && imagesCount === propertyImagesCount) {
        alert('You have to maitain atleast one existing image')
      }
  ...
  handleDelete = () => {
    if (this.state.imagesCount === this.state.propertyImagesCount) {
      ...
      if (this.state.deleteIDs && this.state.deleteIDs.length > 0) {
        let obj = { url: config.baseUrl + config.putSPPropertyImagesDeleteAPI, body: updateObj }
        APICallManager.putCall(obj, function (resObj) {
          if (resObj.data.statusCode === '0000') {
            alert(t`lanSPSuccessPropertyImagesDeleted`)
          } else {
            alert(t`lanSPErrorPropertyImagesDeleteFailed`)
          }
        })
      }
      ...
      async.series([
        ...
        function (callback) {
          ...
          for (let k = 0; k < filesSize; k++) {
            if (files[k]) {
              newNewFiles[indexPos.toString()] = files[k]
              indexPos++
            }
          }
        ...
        _this.props.imageFunction(imageData, newNewFiles, video, videoFiles)
      })
    }
  }
...
SPPropertyImagesEdit.propTypes = {
