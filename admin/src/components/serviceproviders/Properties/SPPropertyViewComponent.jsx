...
class SPPropertyViewComponent extends React.Component {
  ...
  componentWillMount () {
    ...
    if (propertyData && propertyData._id) {
      ...
      let obj = { url: config.baseUrl + config.getSPPropertyDocsAPI + propertyData._id }
      let _this = this
    ...
    if (this.state.videoOld && this.state.videoOld.length > 0) {
      ...
      reader.onloadend = function (e) {
        if (file.size > 150000000) {
          this.setState({ videoErrorMessage: 'Uploaded video size should be less than or equal to 250MB and Duration should be less than or equal to 60 seconds.' })
        } else {
          let media = new Audio(reader.result)
          media.onloadedmetadata = function () {
            if (media.duration > 60) {
              _this.setState({ videoErrorMessage: 'Uploaded video duration should be less than or equal to 60 seconds and Size should be less than or equal to 250MB.' })
            }
          }
        }
        this.setState({
        ...
        fetchPolyfill(config.baseUrl + config.putSPPropertyUpdateAPI + _this.state.propertyId, {
          ...
          response.json().then((responseJson) => {
            if (responseJson.statusCode === '0000') {
              ...
              alert(t`lanSPErrorPropertyUpdateFailed`)
            }
          })
        })
      })
  ...
  imageFunction = (images, files, video, videoFiles) => {
    // let images = []
    // let video = []
    // photos.forEach(item => {
    ...
    this.setState({ images: images, newFiles: files, video: video, videoFiles: videoFiles, videoOld: videoOld, photosStatus: false, reload: true })
  }
                      ...
                      <div className='accordion' id='accordionExample'>
                        ...
                        <div className='card'>
                          ...
                          <div id='collapseThree' className='collapse' aria-labelledby='headingThree' data-parent='#accordionExample'>
                            <div className='card-body'>
                              <SPPropertyNearestAreasUpdate nearestAreas={this.state.nearestAreas} commonFunction={this.commonFunction} />
                            </div>
                          </div>
                        </div>
                        <div className='card'>
