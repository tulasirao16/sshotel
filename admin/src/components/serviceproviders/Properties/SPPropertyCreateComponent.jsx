...
class SPPropertyCreateComponent extends React.Component {
  ...
  imageFunction = (images, files, video, videoFiles) => {
    ...
    //   }
    // })
    this.setState({ images: images, files: files, video: video, videoFiles: videoFiles, photosStatus: false, reload: true })
  }
  ...
  onFileChangeVideo = (e) => {
    ...
    reader.onloadend = function (e) {
      if (file.size > 150000000) {
        this.setState({ videoErrorMessage: t`lanSPErrorVideoSize` })
      } else {
        let media = new Audio(reader.result)
        media.onloadedmetadata = function () {
          if (media.duration > 60) {
            _this.setState({ videoErrorMessage: t`lanSPErrorVideoTime` })
          }
        }
      }
      this.setState({
        ...
        fetchPolyfill(config.baseUrl + config.postSPPropertyCreateAPI, {
          ...
          response.json().then((responseJson) => {
            if (responseJson.statusCode === '0000') {
              toast.success(t`lanSPSuccessPropertyCreatedSuccessfully`, {
              ...
              toast.error(t`lanSPErrorPropertyCreateFailed`, {
                position: toast.POSITION.TOP_CENTER
              })
              // alert(t`lanSPErrorPropertyCreateFailed`)
            }
          })
        })
      })
        ...
        <div className='container-fluid mt--6'>
          <div className='card property-view'>
            ...
            <div className='card-body'>
              <div>
                {this.state.photosStatus ? <SPPropertyImages photos={this.state.images} files={this.state.files} imageFunction={this.imageFunction} video={this.state.video} videoFiles={this.state.videoFiles} />
                : <div>
                  <div className='row'>
                    <div className='col-md-6 create-prop-slider image-col'>
                      ...
                      <div className='add-image-btn'>
                        ...
                        <label className='btn btn-primary btn-file-input add-icon'><i className='far fa-images' />{''} {t`lanSPButtonAddPhotos`}</label>
                      </div>
                    </div>
                      ...
                      <div className='accordion' id='accordionExample'>
                        ...
                        <div className='card'>
                          ...
                          <div id='collapseThree' className='collapse' aria-labelledby='headingThree' data-parent='#accordionExample'>
                            <div className='card-body'>
                              <SPPropertyNearestAreasCreate commonFunction={this.commonFunction} nearestAreas={this.state.nearestAreas} />
                            </div>
                          </div>
                        </div>
                        <div className='card'>
                  ...
                  <div className='row'>
                    <div className='col-sm-12 text-center mt-2'>
                      ...
                      ? <button className='btn btn-primary' type='button' onClick={this.handlePropertyCreate}>{t`lanCommonButtonCreate`}</button> : null}
                      <ToastContainer rtl />
                    </div>
                  </div>
                </div>}
