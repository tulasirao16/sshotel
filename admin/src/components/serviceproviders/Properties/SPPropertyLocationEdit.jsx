...
class SPPropertyLocationEdit extends React.Component {
  constructor (props) {
    super(props)
  ...
  handleUpdateLocationDetails () {
    ...
    if (!this.state.address.trim()) {
      ...
      let obj = { url: config.baseUrl + config.putSPPropertyLocationUpdateAPI + this.state.propertyId, body: putLocData }
      APICallManager.putCall(obj, function (resObj) {
        if (resObj.data.statusCode === '0000') {
          localStorage.setItem('propertyData', JSON.stringify(resObj.data.statusResult))
          // alert(t`lanSPLabelSuccessPropertyLocationUpdated`)
          toast.success(t`lanSPLabelSuccessPropertyLocationUpdated`, {
            position: toast.POSITION.TOP_CENTER
          })
        } else {
          toast.error(t`lanSPLabelErrorPropertyLocationUpdateFailed`, {
            position: toast.POSITION.TOP_CENTER
          })
          // alert(t`lanSPLabelErrorPropertyLocationUpdateFailed`)
        }
      })
    }
  }
        ...
        <form role='form'>
          ...
          <div>
            ...
            <div className='text-center'>
              <button type='button' className='btn btn-primary mt-2' onClick={this.handleUpdateLocationDetails}>{t`lanCommonButtonUpdate`}</button>
              <ToastContainer rtl />
            </div>
          </div>
        </form>
