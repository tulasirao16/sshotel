...
import SPServiceViewEditComponent from './SPServiceViewEditComponent'
...
class SPServicesListComponent extends React.Component {
  constructor (props) {
    super(props)
    this.state = {
      checked: false,
  ...
  componentWillMount () {
    let getSpPropertyInfoAmenities = {
      url: config.baseUrl + config.getSPPropertyInfoServicesListAPI + this.state.propertyId + '/' + this.state.propertyInfoId
    }
    let _this = this
  ...
  handleStatusChange (status, data, i) {
    ...
    if (status) {
      ...
      let putSPPropertyInfoServiceStatus = {
        url: config.baseUrl + config.putSPPropertyInfoServiceActivateAPI + data._id,
        body: {
      ...
      let putSPPropertyInfoServiceStatus = {
        url: config.baseUrl + config.putSPPropertyInfoServiceInactivateAPI + data._id,
        body: {
          propertyInfoId: this.state.propertyInfoId,
          servicesAvailable: servicesAvailable
        }
    ...
    return (
      <div>
        ...
        {!this.state.isServiceView
          ...
          : <SPServiceViewEditComponent selectedService={this.state.selectedService} handleServiceModifiedData={this.handleServiceModifiedData} />
        }
      </div>
    )
  }
...
SPServicesListComponent.propTypes = {
  propertyInfoId: PropTypes.any,
  propertyId: PropTypes.any
}

export default SPServicesListComponent
