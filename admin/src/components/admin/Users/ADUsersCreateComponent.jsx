...
class ADUserCreateComponent extends React.Component {
  constructor (props) {
  ...
  handleCreateUser () {
    ...
    if (!this.state.firstName.trim()) {
      ...
      let _this = this
      let obj = { url: config.baseUrl + config.postADUserCreateAPI, body: userData }
      APICallManager.postCall(obj, function (resObj) {
        if (resObj.data.statusCode === '0000') {
          _this.props.toastFunction('Successfully')
          setTimeout(() => {
            _this.props.handleCreateUserData(resObj.data.statusResult)
          }, 2000)
        } else {
            ...
            <div className='row'>
              ...
              <div className='col-md-3 pr-1'>
                <div className='form-group'>
                  ...
                  <input type='text' className='form-control' name='zip' maxLength='6' onKeyPress={this.handleMobileNumKeys}
                    value={this.state.zip} onChange={() => this.setState({ zip: event.target.value, errorMessage: '' })} />
                </div>
              </div>
            </div>
            <div className='row'>
...
export default ADUserCreateComponent

ADUserCreateComponent.propTypes = {
