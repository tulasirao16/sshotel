...
class ADEUUserProfileIdProofCreateComponent extends React.Component {
  constructor () {
    super()
  ...
  handleIdTypeValidation (event) {
    let euUserData = JSON.parse(localStorage.getItem('euUserData'))
    let euUserId = euUserData._id
    let idType = event.target.value
    let userObj = {
      url: config.baseUrl + config.getADEUProfileIdTypeValidateAPI + idType + '/' + euUserId
    }
    let _this = this
  ...
  handleCreateIdProof () {
    if (!this.state.euUserData.name) {
      this.props.toastFunc('Update Your Profile')
      ...
      let name = this.state.euUserData.name
      let _this = this
      ...
      data.append('euUserId', this.state.euUserData._id)
      this.refs.btn.setAttribute('disabled', 'disabled')
      fetchPolyfill(config.baseUrl + config.postADEUUserIDProofCreateAPI, {
        method: 'POST',
        body: data,
        ...
        response.json().then((resData) => {
          if (resData.statusCode === '0000') {
            ...
            _this.props.toastFunc('ID Proof Created Successfully')
            setTimeout(() => {
              _this.props.handleIdProofCreate(resData.statusResult)
            }, 1000)
          } else {
            _this.props.toastFunc('ID Proof Create Failed')
            setTimeout(() => {
              _this.props.handleIdProofCreate(resData.statusResult)
            }, 2000)
          }
...
ADEUUserProfileIdProofCreateComponent.propTypes = {
  handleIdProofCreate: PropTypes.func,
  toastFunc: PropTypes.any
}

export default ADEUUserProfileIdProofCreateComponent
