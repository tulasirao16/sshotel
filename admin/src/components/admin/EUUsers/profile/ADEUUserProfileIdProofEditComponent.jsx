...
class ADEUUserProfileIdProofEditComponent extends React.Component {
  constructor () {
    super()
    this.state = {
  ...
  handleUpdateIdProof () {
    ...
    if (!this.state.idTypeValid) {
      ...
      if (!isUpdate || this.state.filename) {
        ...
        this.props.toastFunc('No Changes to Update')
        setTimeout(() => {
  ...
  handleApiCall () {
    ...
    fetchPolyfill(config.baseUrl + config.putADEUUserProfileIDProofUpdateAPI + this.state._id, {
      method: 'PUT',
      body: data,
      ...
      response.json().then((resData) => {
        if (resData.statusCode === '0000') {
          this.props.toastFunc('Id Proof Updated Successfully')
          setTimeout(() => {
            this.props.handleHome()
          }, 2000)
        } else {
          this.props.toastFunc('Id Proof Update Failed')
        }
      })
    })
  }
...
ADEUUserProfileIdProofEditComponent.propTypes = {
  handleHome: PropTypes.func,
  toastFunc: PropTypes.any
}

export default ADEUUserProfileIdProofEditComponent
