...
class ADProfileIdProofCreateComponent extends React.Component {
  constructor () {
    super()
    ...
    APICallManager.getCall(userObj, function (resObj) {
      if (resObj.data.statusCode === '0000') {
        ...
        _this.setState({
          ...
          nameOnId: idProofData && idProofData.nameOnId ? idProofData.nameOnId : '',
  ...
  handleCreateIdProof () {
    if (this.state.idProofSatus === true) {
      this.setState({ errorMessage: 'This id  type already exist' })
    } else if (!this.state.authObj.name) {
      ...
      this.setState({ errorMessage: 'Please choose ID Proof Image' })
      ...
      fetchPolyfill(config.baseUrl + config.postADEUUserProfileIdProofCreateAPI, {
        method: 'POST',
        body: data,
          ...
          <form>
            ...
            <div className='row'>
              <div className='col-sm-12 text-center'>
                <button type='button' ref='btn' className='btn btn-primary' onClick={this.handleCreateIdProof} >Create</button>
              </div>
            </div>
          </form>
        </div>
...
ADProfileIdProofCreateComponent.propTypes = {
  handleIdProofCreate: PropTypes.func
}

export default ADProfileIdProofCreateComponent
