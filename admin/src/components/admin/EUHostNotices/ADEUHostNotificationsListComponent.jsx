...
import ADEUHostNotificationsListViewComponent from './ADEUHostNotificationsListViewComponent'
...
class ADEUHostNotificationsListComponent extends React.Component {
  constructor (props) {
  ...
  handleInputChange (event) {
    ...
    let obj = { url: config.baseUrl + config.getADEUHostNotificationsAPI + this.state.userData._id + '/' + this.state.activePage + '/' + search }
    let _this = this
    APICallManager.getCall(obj, function (resObj) {
      if (resObj.data.statusCode === '0000') {
        _this.setState({
  ...
  handlePageChange (pageNumber) {
    ...
    APICallManager.getCall(obj, function (resObj) {
      if (resObj.data.statusCode === '0000') {
        ...
        _this.setState({ notificationsList: [], totalCount: 0, nomatchesData: false })
      }
    })
  }
  handleStatus (item) {
    ...
    APICallManager.putCall(obj, function (resObj) {
      if (resObj.data.statusCode === '0000') {
        ...
        ToastsStore.warning('Notification Unread failed')
      }
    })
  }
  handleDelete (item) {
    ...
    return (
      <div>
        {this.state.isShowNotification === 'List'
                                ...
                                : <div className='container'>
                                  <div className='row justify-content-center'>
                                    <div className='col-sm-12 text-center'>
                                      <div className='no-data'><p>{t`lanCommonLabelNoMatchesFound`}</p>
                                      </div>
                                    </div>
                                  </div>
                                </div>
          ...
          : this.state.isShowNotification === 'View' ? <ADEUHostNotificationsListViewComponent notificationData={this.state.notificationData} handleViewUser={this.handleViewNotificationBack} />
            : ''}
...
export default ADEUHostNotificationsListComponent
