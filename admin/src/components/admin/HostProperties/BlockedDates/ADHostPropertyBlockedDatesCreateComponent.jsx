...
class ADHostPropertyBlockedDatesCreateComponent extends React.Component {
  constructor (props) {
    super(props)
    this.state = {
    ...
    if (this.state.blockingType === 'Continuous Blocking' && !this.state.from) {
      ...
      if (this.state.blockingType === 'Continuous Blocking') {
        postData = {
          ...
          spLocationId: this.state.propertyObj.spLocationId._id,
          spServiceProviderId: this.state.propertyObj.spServiceProviderId,
          spServiceProvider: this.state.propertyObj.spServiceProvider
        }
      } else {
        postData = {
          blockingType: this.state.blockingType,
      ...
      let obj = { url: config.baseUrl + config.postADHostPropertyBlockedDatesCreateAPI, body: postData }
      let _this = this
      APICallManager.postCall(obj, function (resObj) {
        _this.setState({ blockingType: 'Continuous Blocking', from: '', to: '', dates: [], customDatesStyles: [] })
        if (resObj.data.statusCode === '0000') {
          localStorage.setItem('PropertiesShowfromBD', 'View')
...
ADHostPropertyBlockedDatesCreateComponent.propTypes = {
  propertyObj: PropTypes.any,
  blockedDatesObj: PropTypes.any
}

export default ADHostPropertyBlockedDatesCreateComponent
