...
class ADHostsAminitiesViewComponent extends React.Component {
  constructor (props) {
    super(props)
    this.state = {
      date: new Date(),
  ...
  handlAmenityEdit () {
    let amenityObj = {
      _id: this.state.amenitiesData._id,
    ...
    if (this.state.amenityStatus === this.state.amenitiesData.amenityStatus && this.state.amenityType === this.state.amenitiesData.amenityType && this.state.charge === this.state.amenitiesData.amenityCharge) {
      this.props.handleAminityViewCreate(amenityObj, false)
    } else if (this.state.amenityType === 'Paid' && this.state.charge === '0') {
      this.setState({ errorMessage: t`lanSPLabelErrorAmenityCharged` })
    } else {
      let putSPPropertyInfoAmenity = {
        url: config.baseUrl + config.putSPPropertyInfoAmenitiesUpdateAPI,
        body: amenityObj
      }
      let _this = this
      APICallManager.putCall(putSPPropertyInfoAmenity, function (resObj) {
        if (resObj.data.statusCode === '0000') {
          _this.props.handleAminityViewCreate(amenityObj, true)
        } else {
...
ADHostsAminitiesViewComponent.propTypes = {
  propertyId: PropTypes.any,
  propertyInfoId: PropTypes.any,
  // handleAmenityData: PropTypes.any,
  handleAminityViewCreate: PropTypes.any,
  amenitiesObj: PropTypes.any,
  handleAminityViewBack: PropTypes.any
}
export default ADHostsAminitiesViewComponent
