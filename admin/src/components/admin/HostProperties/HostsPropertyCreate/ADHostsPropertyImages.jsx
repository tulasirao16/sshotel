...
export default class ADHostsPropertyImages extends React.Component {
  constructor (props) {
    super(props)
    this.state = {
      photos: props.photos,
  ...
  handleBack = () => {
    this.props.imageFunction(this.state.photos, this.state.files, this.state.video, this.state.videoSelectNDel ? true : this.state.videoError)
  }
  handleSelect (i) {
      ...
      <div className='container view-property-files'>
        <div className='row'>
          {this.state.photos && this.state.photos.length > 0 ? this.state.photos.map((item, i) => {
            return (
              <SPPropertyImageView image={item} key={i} handleSelect={this.handleSelect} selected={false} />
            )
          })
...
ADHostsPropertyImages.propTypes = {
