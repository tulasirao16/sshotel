...
class ADHostsPropertyLocationCreate extends React.Component {
  constructor (props) {
    super(props)
    let authObj = JSON.parse(localStorage.getItem('authObj'))
    let hostData = JSON.parse(localStorage.getItem('hostData'))
    let mobileNumber = hostData ? hostData.contactNumber : ''
    let alternateMobileNumber = hostData && hostData.alternateMobileNumber ? hostData.alternateMobileNumber : ''
    let email = hostData ? hostData.contactEmail : ''
    let spServiceProvider = hostData ? hostData.spServiceProvider : ''
    let contactPerson = hostData ? hostData.contactPerson : ''
    this.state = {
      authObj: authObj,
  ...
  componentWillMount () {
    ...
    let hostData = JSON.parse(localStorage.getItem('hostData'))
    let spServiceProviderId = hostData._id
    this.state.spServiceProviderId = spServiceProviderId
    let obj = {
      url: config.baseUrl + config.getADHostsPropertyLocationsListAPI + spServiceProviderId
    }
    let _this = this
  ...
  handleLocations (event) {
    ...
    if (event.target.value === 'Select Locations List') {
      ...
      if (hostData && hostData._id) {
        this.setState({
          locationValue: event.target.value,
          contactPerson: hostData.contactPerson,
          mobileNumber: hostData.contactNumber,
          alternateMobileNumber: hostData && hostData.alternateMobileNumber ? hostData.alternateMobileNumber : '',
          email: hostData.contactEmail,
          spServiceProvider: hostData.spServiceProvider
        })
      }
    }
  }
            ...
            <div className='col-md-6 room-catageory'>
              <div className='form-group'>
                <div className='custom-control custom-radio'>
                  <input value='Create' type='radio' id='CreateLocation' checked={this.state.locationValue === 'Create'}
                    onChange={this.handleLocations} />
                  <label htmlFor='CreateLocation'>{t`lanSPButtonCreateLocation`}</label>
                </div>
                <div className='custom-control custom-radio'>
                  <input value='Select Locations List' id='SelectLocation' type='radio' checked={this.state.locationValue === 'Select Locations List'} onChange={this.handleLocations} />
                  <label htmlFor='SelectLocation'>{t`lanSPLabelSelectLocationsList`}</label>
                </div>
              </div>
            </div>
...
ADHostsPropertyLocationCreate.propTypes = {
  commonFunction: PropTypes.any,
  spLocationObj: PropTypes.any
}
export default ADHostsPropertyLocationCreate
