...
import ADHostsAmenitiesViewComponent from './ADHostsAmenitiesViewComponent'
...
class ADHostsAmenitiesListComponent extends React.Component {
  constructor (props) {
    super(props)
    this.state = {
      reload: false,
  ...
  componentWillMount () {
    ...
    let getADHostsPropertyInfoAmenities = {
      url: config.baseUrl + config.getADHostsPropertyInfoAmenitiesListAPI + this.props.propertyInfoId + '/' + this.state.searchString
    }
    let _this = this
    APICallManager.getCall(getADHostsPropertyInfoAmenities, function (resObj) {
  ...
  handleStatusChange (status, data, i) {
    ...
    switch (status) {
      case false :
        ...
        let putADHostsPropertyInfoAmenityUnavailable = {
          url: config.baseUrl + config.putADHostsPropertyInfoAmenitiesStatusAPI,
          body: {
            _id: data._id,
        ...
        APICallManager.putCall(putADHostsPropertyInfoAmenityUnavailable, function (resObj) {
          if (resObj.data.statusCode === '0000') {
      ...
      case true :
        _this.state.availableAmenities.push(data.amenityName)
        let putADHostsPropertyInfoAmenityAvailable = {
          url: config.baseUrl + config.putADHostsPropertyInfoAmenitiesStatusAPI,
          body: {
            _id: data._id,
        ...
        APICallManager.putCall(putADHostsPropertyInfoAmenityAvailable, function (resObj) {
          if (resObj.data.statusCode === '0000') {
  ...
  handleSearch = (event) => {
    ...
    let searchAmenities = this.state.searchAmenitiesData.filter(function (item) {
      return item.amenityName.toLowerCase().indexOf(event.target.value.toLowerCase()) > -1 ||
      item.amenityType.toLowerCase().indexOf(event.target.value.toLowerCase()) > -1 ||
        item.amenityStatus.toLowerCase().indexOf(event.target.value.toLowerCase()) > -1
    })
  ...
  handleAmenityData (amenityObj, modified) {
    if (modified) {
      ...
      const index = amenitiesData.findIndex(dataObj => dataObj._id === amenityObj.recordID)
      let isModifiedStatus = this.state.amenitiesData[index].amenityStatus === amenityObj.amenityStatus ? 'Unavailable' : 'Available'
  ...
  render () {
    return (
      ...
      // : ''
      : <ADHostsAmenitiesViewComponent amenitiesObj={this.state.amenitiesObj} propertyInfoId={this.state.propertyInfoId} propertyId={this.state.propertyId} handleAmenityData={this.handleAmenityData} />
    )
  }
}

ADHostsAmenitiesListComponent.propTypes = {
  propertyInfoId: PropTypes.any,
  propertyId: PropTypes.any
}

export default ADHostsAmenitiesListComponent
