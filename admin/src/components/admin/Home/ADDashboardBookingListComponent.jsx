...
import ADBookingsViewRatings from './../EUBookingsHistory/GiveRating'
...
class ADDashboardBookingsList extends React.Component {
  constructor () {
    super()
    this.state = {
      bookingData: [],
  ...
  componentWillMount () {
    ...
    let type = localStorage.getItem('bookingStatus')
    let getAdBookingsObj = {}
    switch (type) {
      case 'CheckIn':
        getAdBookingsObj = {
          url: config.baseUrl + config.getADHomeCheckInBookingsListAPI + this.state.activePage + '/' + this.state.searchString
        }
        break
      case 'CheckOut':
        getAdBookingsObj = {
          url: config.baseUrl + config.getADHomeCheckOutBookingsListAPI + this.state.activePage + '/' + this.state.searchString
        }
        break
      case 'Bookings':
        this.setState({ heading: 'Bookings' })
        getAdBookingsObj = {
          url: config.baseUrl + config.getADHomeTodayBookingsListAPI + this.state.activePage + '/' + this.state.searchString
        }
        break
      case 'Cancelled':
        this.setState({ heading: 'Cancelled Bookings' })
        getAdBookingsObj = {
          url: config.baseUrl + config.getADHomeCancelledBookingsListAPI + this.state.activePage + '/' + this.state.searchString
        }
        break
      case 'Amounts':
        this.setState({ heading: 'Booking Amounts' })
        getAdBookingsObj = {
          url: config.baseUrl + config.getADHomeAmountsBookingsListAPI + this.state.activePage + '/' + this.state.searchString
        }
        break
    }
  ...
  handleInputChange (event) {
    ...
    switch (type) {
      case 'CheckIn':
        getAdBookingsObj = {
          url: config.baseUrl + config.getADHomeCheckInBookingsListAPI + '1' + '/' + this.state.searchString
        }
        break
      case 'CheckOut':
        getAdBookingsObj = {
          url: config.baseUrl + config.getSDHomeCheckOutBookingsListAPI + '1' + '/' + this.state.searchString
        }
        break
      case 'Bookings':
        this.setState({ heading: 'Bookings' })
        getAdBookingsObj = {
          url: config.baseUrl + config.getADHomeTodayBookingsListAPI + '1' + '/' + this.state.searchString
        }
        break
      case 'Cancelled':
        this.setState({ heading: 'Cancelled Bookings' })
        getAdBookingsObj = {
          url: config.baseUrl + config.getADHomeCancelledBookingsListAPI + '1' + '/' + this.state.searchString
        }
        break
      case 'Amounts':
        this.setState({ heading: 'Booking Amounts' })
        getAdBookingsObj = {
          url: config.baseUrl + config.getADHomeAmountsBookingsListAPI + '1' + '/' + this.state.searchString
        }
        break
    }
    this.handleAPICall(getAdBookingsObj)
  ...
  handlePageChange (pageNumber) {
    if (this.state.activePage !== pageNumber) {
      ...
      switch (type) {
        case 'CheckIn':
          getAdBookingsObj = {
            url: config.baseUrl + config.getADHomeCheckInBookingsListAPI + pageNumber + '/' + this.state.searchString
          }
          break
        case 'CheckOut':
          getAdBookingsObj = {
            url: config.baseUrl + config.getADHomeCheckOutBookingsListAPI + pageNumber + '/' + this.state.searchString
          }
          break
        case 'Bookings':
          this.setState({ heading: 'Bookings' })
          getAdBookingsObj = {
            url: config.baseUrl + config.getADHomeTodayBookingsListAPI + pageNumber + '/' + this.state.searchString
          }
          break
        case 'Cancelled':
          this.setState({ heading: 'Cancelled Bookings' })
          getAdBookingsObj = {
            url: config.baseUrl + config.getADHomeCancelledBookingsListAPI + pageNumber + '/' + this.state.searchString
          }
          break
        case 'Amounts':
          this.setState({ heading: 'Booking Amounts' })
          getAdBookingsObj = {
            url: config.baseUrl + config.getADHomeAmountsBookingsListAPI + pageNumber + '/' + this.state.searchString
          }
          break
      }
  ...
  handleBack () {
    if (this.state.viewRatings) {
      this.setState({ viewRatings: false })
    } else {
  ...
  handleBookingView (data) {
    localStorage.setItem('bookingsby', 'dashboard')
  ...
  handleBookingCancel (data) {
    ...
    APICallManager.getCall(getADBookingsObj, function (resObj) {
      if (resObj.data.statusCode === '0000') {
        ...
        ToastsStore.success(t`Sorry! You cant cancel this booking`)
      }
    })
  }
      ...
      case '0000' :
        ...
        APICallManager.putCall(obj, function (resObj) {
          if (resObj.data.statusCode === '0000') {
            ...
            ToastsStore.warning('Booking Cancellation Failed')
          }
        })
        break
      case '8888' :
  ...
  render () {
    return (
                              ...
                              <table className='table align-items-center table-flush table-striped'>
                                ...
                                </thead>
                                {
                                  this.state.bookingData.map((item, i) =>
                                    <tbody key={i}>
                                      <tr>
                                        {this.state.bookedByChecked ? <td >{item.euName}</td> : null}
                                        {this.state.mobileNumberChecked ? <td >{item.euMobileNumber}</td> : null}
                                        {this.state.emailChecked ? <td >{item.euEmail}</td> : null}
                                        {this.state.checkInChecked ? <td >{moment(item.checkInDate).format('MMM DD,YY(hh:mm A)')}</td> : null}
                                        {this.state.checkOutChecked ? <td >{moment(item.checkOutDate).format('MMM DD,YY(hh:mm A)')}</td> : null}
                                        {this.state.noOfRoomsChecked ? <td >{item.noOfRooms}</td> : null}
                                        {this.state.propertyTitleChecked ? <td >{item.spPropertyTitle}</td> : null}
                                        {this.state.areaChecked ? <td >{item.spLocationObj.area}</td> : null}
                                        {this.state.bookingCodeChecked ? <td >{item.bookingCode}</td> : null}
                                        {this.state.bookingAmountChecked ? <td >{item.spTotalAmount}</td> : null}
                                        {this.state.statusChecked ? <td >{item.bookingStatus}</td> : null}
                                        <td className='table-actions'>
                                          ...
                                          <div className='table-actions btn-actions'>
                                            ...
                                            {item.bookingStatus === 'Completed' || item.bookingStatus === 'Closed' || item.bookingStatus === 'Checked-Out'
                                              ? <a onClick={() => this.handleBookingRating(item)} className='table-action table-action-review' data-toggle='tooltip'
                                                data-placement='top' title={t`lanCommonButtonTooltipReviews`}>
      ...
      : <ADBookingsViewRatings hotel={this.state.hotel} bookingCode={this.state.bookingCode} data={this.state.data} handleViewClose={this.handleBack} screen='Dashboard' /> }
...
export default ADDashboardBookingsList
