    ...
    if (!this.state.password) {
      ...
      APICallManager.postCall(obj, function (resObj) {
        ...
        if (resObj.data.statusCode === '0000') {
          ...
          ToastsStore.success('You have signed up successfully')
          setTimeout(() => {
                ...
                <form role='form'>
                  ...
                  <div className='px-lg-7 '>
                    <div className='form-group'>
                      ...
                      <input autoFocus className='form-control tel-input eu-font' placeholder={t`lanEULabelEnterPassword`} style={this.state.pSuccess ? { borderColor: '#4da424' } : {}}
                        type={this.state.showPassword ? 'password' : 'text'} value={this.state.password} maxLength={20}
                        onKeyDown={this.handleEnter} onChange={(e) => this.setState({ password: e.target.value, pError: '', pSuccess: e.target.value.length > 5 })} />
                  ...
                  <div>
                    <div className='text-center'>
                      <button type='button' className='btn btn-primary mt-2' disabled={this.state.submitDisabled} onClick={this.handleSignupSubmit}>{ t`lanEUButtonSubmit` }</button>
                    </div>
                  </div>
                </form>
              </div>
