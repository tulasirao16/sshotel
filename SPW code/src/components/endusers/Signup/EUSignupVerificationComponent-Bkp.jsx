...
class EUSignupVerificationComponent extends React.Component {
  ...
  handleSubmit (event) {
    if (!this.state.userID.trim()) {
      this.setState({ uError: t`lanEULabelErrorPleaseEnterUserId`, uSuccess: false })
    } else if (!this.state.mobile.trim()) {
      this.setState({ mError: t`lanEULabelErrorPleaseEnterMobileNumber`, mSuccess: false })
    } else if (!this.state.mobile.trim().match(phRegex)) {
      this.setState({ mError: t`lanEULabelErrorInvalidMobileNumber`, mSuccess: false })
    } else if (!this.state.email.trim()) {
      this.setState({ eError: t`lanEULabelErrorPleaseEnterEmail`, eSuccess: false })
    } else if (!this.state.email.trim().match(emailRegex)) {
      this.setState({ eError:  t`lanEULabelErrorInvalidEmail`, eSuccess: false })
    } else if (!this.state.password) {
      this.setState({ pError:  t`lanEULabelErrorPleaseEnterPassword`, pSuccess: false })
    } else if (this.state.password.length < 6) {
      this.setState({ pError:  t`lanEULabelErrorPasswordShouldContainMinimumSixCharacters`, pSuccess: false })
    } else if (!this.state.mVerifyStatus) {
      this.setState({ svErrorMessage:  t`lanEULabelErrorPleaseVerifyMobileNumber` })
    } else if (!this.state.eVerifyStatus) {
      this.setState({ svErrorMessage:  t`lanEULabelErrorPleaseVerifyEmail` })
    } else {
                  ...
                  <form role='form'>
                    ...
                    <div className='px-lg-6 '>
                      <div className='form-group'>
                        <label className='form-control-label'>{t`lanEULabelUserID`}</label>
                      ...
                      <div className='form-group'>
                        <label className='form-control-label'>{t`lanEULabelMobile`}</label>
                      ...
                      <div className='form-group'>
                        <label className='form-control-label'>{t`lanEULabelEmail`}</label>
                      ...
                      <div className='form-group'>
                        <label className='form-control-label'>{t`lanEULabelPassword`}</label>
                    ...
                    <div>
                      <div className='text-center'>
                        <button type='button' className='btn btn-primary mt-2' onClick={this.handleBack}>{t`lanEUButtonBack`}</button>
                        <button type='button' className='btn btn-primary mt-2' onClick={this.handleSubmit}>{t`lanEUButtonDone`}</button>
                      </div>
                    </div>
                  </form>
                </div>
            ...
            <div className='modal-content'>
              <div className='modal-body'>
                ...
                <div className='text-center mb-3'>
                  ...
                  <p>{t`lanEULabelPleaseEnterTheFourDigitOTPNumberBelow`}</p>
                </div>
                ...
                <div className='form-row mt-4 text-center'>
                  <div className='col'>
                    <button type='button' ref='mobileOtpSubmit' className='btn btn-primary mr-3'>{t`lanEUButtonSubmit`}</button>
                    <button type='button' ref='mobileOtpClose' className='btn btn-outline-default' data-dismiss='modal' aria-label='Close'>{t`lanEUButtonClose`}</button>
                  </div>
                </div>
                <div className='text-center mt-4'>
                  <div className='h4 font-weight-300'>
                    <p>{t`lanEULabelOTP`} : 8839</p>
                    <a href='#' className='btn-link'>{t`lanEULabelResendOTP`}</a>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className='modal-dialog modal- modal-dialog-centered modal-sm' role='document'>
            <div className='modal-content'>
              <div className='modal-body'>
                ...
                <div className='form-row mt-4 text-center'>
                  <div className='col'>
                    <button type='button' className='btn btn-primary mr-3'>{t`lanEUButtonSubmit`}</button>
                    <button type='button' className='btn btn-outline-default'>{t`lanEUButtonClose`}</button>
                  </div>
                </div>
                <div className='text-center mt-4'>
                  <div className='h4 font-weight-300'>
