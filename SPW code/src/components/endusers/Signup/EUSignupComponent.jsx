...
class EUSignupComponent extends React.Component {
  ...
  handleSpVerify (event) {
    ...
    if (authObj && authObj.userType) {
      ...
      APICallManager.postCall(obj, function (resObj) {
        _this.setState({ nextDisabled: false })
  ...
  handleNumber (event) {
    if ((event.charCode >= 32 && event.charCode < 48) ||
    (event.charCode > 57 && event.charCode < 127) || (event.charCode === 13)) {
      if (event.charCode === 13) {
        this.handleSpVerify(event)
      }
      event.preventDefault()
    }
  }
                              ...
                              <div className='row'>
                                <div className='col-sm-12 text-left'>
                                  <div className='form-group'>
                                    ...
                                    <input type='text' autoFocus className='form-control' placeholder={t`lanEULabelEnterMobileNumber`} style={this.state.mbnSuccess ? { borderColor: '#4da424' } : {}} maxLength={10}
                                      onKeyPress={this.handleNumber} onChange={(e) => this.setState({ mobileNumber: e.target.value, mbnError: '', errorMessage: '', mbnSuccess: e.target.value.length > 9 })}
                                ...
                                <div className='col-lg-12 col-sm-12 text-right'>
                                  ...
                                  <div className='text-right mt-2'>
                                    <span className='text-muted text-sm'><small>{ t`lanEULabelDontHaveAccount` } </small>
                                      <a style={{ color: '#01a4a2' }} onClick={this.handleEndUserLogin} className='text-underline'> { t`lanEUButtonLogin` }</a>
                                    </span>
