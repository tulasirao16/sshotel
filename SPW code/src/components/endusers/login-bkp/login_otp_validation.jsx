...
class EndUsersLoginOTPValidation extends React.Component {
  ...
  handleSubmit (event) {
    ...
    if (!otpNum) {
      this.setState({ errorMessage: t`lanEULabelErrorOTPIsRequired` })
    } else if (!otpNumValid.test(otpNum)) {
      this.setState({ errorMessage:  t`lanEULabelErrorInvalidOTPNumber` })
    } else {
                        ...
                        <div className='card-group login'>
                          <div className='card'>
                            <div className='card-body'>
                              <h3 className='text-center'>{t`lanEULabelEnterOTPNumber`}</h3>
                              <p className='text-muted'>{/* --Style-- */}</p>
                              <form onSubmit={this.handleSubmit}>
                                <div className='otp_div'>
                                  <p>{t`lanEULabelPleaseEnterTheOTPSentTo`}<strong> {this.state.loginMobile} </strong> </p>
                                  ...
                                  <button type='submit' className='btn btn-primary px-4' >{t`lanEUButtonVerify`}</button>
                                </div>
                              </form>
                            </div>
                          </div>
