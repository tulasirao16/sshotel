...
class EUProfileComponent extends React.Component {
  constructor () {
    ...
    var checkOut = moment.utc(moment().subtract(18, 'years').format('YYYY-MM-DD'))
      ...
      fetchPolyfill(config.baseUrl + config.putEUProfileAPI + '_id', {
        ...
        response.json().then((body) => {
          ...
          if (response.status === 200) {
            ...
            ToastsStore.success(t`lanEULabelErrorProfileUpdatedSuccessfully`)
            ...
            ToastsStore.error(t`lanEULabelErrorProfileUpdateFailed`)
          }
        })
      })
    }
  ...
  handleVerifyMobileNumber () {
    if (!this.state.mobileNumber.trim()) {
      ...
      this.setState({ mobileError: t`lanEULabelErrorInvalidMobileNumber`, mobileSuccess: false })
      ToastsStore.error(t`lanEULabelErrorInvalidMobileNumber`)
    } else {
      let obj = { url: config.baseUrl + config.postProfileMobileNumberUniqnessVerifyAPI, routing: '/profile', body: { mobileNumber: this.state.mobileNumber } }
      let _this = this
      APICallManager.postCall(obj, function (resObj) {
        if (resObj.data.statusCode === '1001') {
          _this.setState({ modalIsOpen: true, otpValue: resObj.data.statusResult.otpNumber })
        } else {
          ToastsStore.error(t`lanEULabelErrorNumberAlreadyExist`)
        }
      })
    }
    event.preventDefault()
  ...
  handleSubmit () {
    ...
    if (!otpNumber) {
      ...
      let obj = { url: config.baseUrl + config.postProfileMobileNumberOTPVerifyAPI, routing: '/profile', body: userData }
      APICallManager.postCall(obj, function (resObj) {
                      ...
                      <form>
                          ...
                          <div className='col-sm-6' >
                            <div className='form-group row'>
                              ...
                              <div className='col-md-8 pt-2'>
                                ...
                                {!this.state.disabled && this.state.mobileVerifyButton === 'true' ? <a style={{ float: 'right', textDecoration: 'underline' }} disabled={this.state.verifyStatus || !this.state.mobileNumber} onClick={this.handleVerifyMobileNumber}>{this.state.verifyStatus ? 'Verified' : 'Verify' }</a> : ''}
                          ...
                          <div className='col-sm-6' >
                            <div className='form-group row'>
                              ...
                              <div className='col-md-8 pt-2'>
                                ...
                                />
                                {/* <i className='far fa-calendar-alt icon-cal eu-pro' /> */}
                              </div>
                            </div>
                          </div>
                          <div className='col-sm-6' >
                        ...
                        ? <div className='row'>
                          <div className='col-sm-12 text-center' >
                            <a disabled={this.state.submitDisabled} onClick={this.handleUpdateUserProfile} className='btn btn-primary btn-text-white btn-center' >{ t`lanCommonButtonUpdate` }</a>
                          </div>
                        </div>
                        : null }
                      </form>
          ...
          <div className='card-body px-lg-3 signup-verify'>
            <form role='form'>
              <div className='text-center mb-3'>
                ...
                <p>{t`lanEUCommonLabelEnterOtp`}</p>
              </div>
