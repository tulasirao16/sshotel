...
class EUProfileChangePasswordComponent extends React.Component {
  ...
  handleUpdate () {
    if (!this.state.currentPassword) {
      ...
      ToastsStore.error(t`lanEULabelErrorEnterCurrentPassword`)
      ...
      APICallManager.putCall(obj, function (res) {
        ...
        if (res.data.statusCode === '1012') {
          localStorage.clear()
          ToastsStore.success(t`lanEULabelSuccesPasswordUpdatedSuccessfully`)
          hashHistory.push('/login')
        } else if (res.data.statusCode === '9979') {
          ToastsStore.error(t`lanEULabelErrorInvalidCurrentPassword`)
        } else {
          ToastsStore.error(t`lanEULabelErrorPasswordUpdateFailed`)
        }
      })
    }
  }
                    ...
                    <div className='card-body '>
                      ...
                      <div className=' row'>
                        <div className='col-sm-12 text-left' >
                          <a className='btn btn-primary text-white' disabled={this.state.submitDisabled} onClick={this.handleUpdate}>{ t`lanCommonButtonUpdate` }</a>
                        </div>
                      </div>
                    </div>
                  </div>
