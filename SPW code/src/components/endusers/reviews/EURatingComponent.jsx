...
class EURatingComponent extends React.Component {
    ...
    if (this.state.rating === 0) {
      ...
      APICallManager.putCall(putEUReviewRating, function (resObj) {
        if (resObj.data.statusCode === '1051') {
          _this.setState({ successMessage: t`lanEULabelSuccesUpdatedSuccessfully` })
          setTimeout(() => {
  ...
  handleCreate () {
    if (this.state.rating === 0) {
      ...
      let _this = this
      let obj = { url: config.baseUrl + config.postEUReviewRatingsAPI, routing: '/bookings', body: postObj }
      APICallManager.postCall(obj, function (resObj) {
        if (resObj.data.statusCode === '1050') {
          _this.setState({ successMessage: t`lanEULabelSuccesCreatedSuccessfully` })
          setTimeout(() => {
            _this.props.handleViewClose()
          }, 500)
        } else {
          _this.setState({ errorMessage: t`lanEULabelErrorReviewRatingFailed` })
        }
      })
    }
  }
                            ...
                            <li className='list-group-item-one py-1'>
                              <div className='row align-items-center'>
                                <div className='col-lg-3'>
                                  <small>{t`lanEULabelProperty`}</small>
                                </div>
                                <div className='col-lg-8'>
                                  <p className='mb-0 card-text'>
                                    {(this.state.reviewRatingData.bookingId && this.state.reviewRatingData.bookingId.spPropertyTitle) ? this.state.reviewRatingData.bookingId.spPropertyTitle : this.props.hotel}, {this.state.reviewRatingData.spLocationId ? this.state.reviewRatingData.spLocationId.area : ''}</p>
                                </div>
                              </div>
                            </li>
                            <li className='list-group-item-one py-1'>
