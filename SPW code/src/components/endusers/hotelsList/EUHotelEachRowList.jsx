...
export default class EUHotelsEachRowListComponent extends React.Component {
  ...
  handleFavouriteProperty = (data) => {
    if (!this.state.authObj.name || !this.state.authObj.mobileNumber) {
      ToastsStore.warning(t`lanEULabelErrorPleaseFillYourProfileInformation`)
  ...
  handleUnFavouriteProperty (data) {
    ...
    APICallManager.putCall(obj, function (resObj) {
      if (resObj && resObj.data.statusCode === '0000') {
        _this.setState({ isFavourite: false, submitDisabled: false })
            ...
            <div className='room'>
              <figure>
                ...
                <div className='img-div-list'>
                  <img src={(data.propertyId && data.propertyId.imagePath) ? config.baseUrl + data.propertyId.imagePath : require('../../../../assets/property_big_05.jpg')} alt='' className='img-fluid img-container' />
                </div>
                {data.pricing.minBasePriceUnit === '6 Hours'
                ? <a onClick={this.handleHourlyBooking}>
              ...
              <div role='button' onClick={() => this.handleHotelListItemView(data)}>
                ...
                <div className='room-icons'>
                  ...
                  ? <div className='room-price'>
                    ...
                    ? <div>
                      <div className='txt1 realp'><del>{data.pricing.weekEndWdTotalPrice}</del></div>
                      <div className='txt2'>
                        <div>{`${data.pricing.weekEndBasePriceDiscount} % off`}</div>
                      </div>
                    </div>
                    : ''}
                  </div>
                  : <div className='room-price'>
                    ...
                    ? <div>
                      ...
                      <div className='txt2'>
                        {`${data.pricing.basePriceDiscount} % off`}
                      </div>
                    </div>
                    : ''}
                  </div>}
