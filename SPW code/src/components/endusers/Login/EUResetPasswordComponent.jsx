  ...
  handleResetPasswordDone (event) {
    if (!this.state.newPassword) {
      ...
      APICallManager.putCall(putObj, function (resObj) {
        _this.setState({ doneDisabled: false })
        try {
                ...
                <div className='card-body px-lg-5 py-lg-3'>
                  <form role='form'>
                      ...
                      <div className='col-md-12 text-left'>
                        <div className='form-group'>
                          ...
                          <input autoFocus type={this.state.showPassword ? 'text' : 'password'} className='form-control' placeholder={t`lanEULabelEnterNewPassword`}
                            onKeyDown={this.handleEnter} value={this.state.newPassword} onChange={(e) => this.setState({ newPassword: e.target.value, errorMessage: '' })} maxLength={20} />
                    ...
                    <div className='text-center'>
                      <button type='button' className='btn btn-primary mt-2' disabled={this.state.doneDisabled} onClick={this.handleResetPasswordDone}>{t`lanCommonButtonDone`}</button>
                    </div>
                  </form>
                </div>
              </div>
