...
class EUForgotPasswordComponent extends React.Component {
  ...
  handleSendOTP (event) {
    ...
    if (authObj && authObj.userRole === 'Customer') {
      ...
      this.setState({ sendOTPDisabled: true })
      let postObj = {
      ...
      APICallManager.postCall(postObj, function (resObj) {
        _this.setState({ sendOTPDisabled: false })
        try {
                ...
                <div className='card-body px-lg-5 py-lg-2'>
                    ...
                    <div className='col-md-12 text-left'>
                      <div className='px-lg-3 '>
                        <div className='form-group'>
                          <label htmlFor='email-phone' className='form-control-label'>{t`lanEULabelEmailOrMobile`}<span className='mandatory'>*</span></label>
                          <input autoFocus type='text' className='form-control eu-font' id='email-phone' maxLength={80}
                            onKeyDown={this.handleEnter} value={this.state.userId} onChange={() => this.setState({ userId: event.target.value, errorMessage:'' })}
                  ...
                  <div className='row'>
                    <div className='col-md-12 text-center'>
                      <div className=' mb-4'>
                        <button type='button' disabled={this.state.sendOTPDisabled} onClick={this.handleSendOTP} className='btn btn-primary'>{t`lanEUButtonSendOtp`}</button>
                      </div>
                    </div>
                  </div>
                </div>
