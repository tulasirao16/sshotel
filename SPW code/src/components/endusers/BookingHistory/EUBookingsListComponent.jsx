...
class EUBookingsListComponent extends React.Component {
  ...
  handleSearch = (event) => {
    ...
    let getEUBookingsObj = {
      unBlockStatus: true,
      url: config.baseUrl + config.getEUBookingsHistoryListAPI + 1 + '/' + this.state.dataType + '/' + event.target.value
    }
    APICallManager.getCall(getEUBookingsObj, function (resObj) {
      if (resObj.data.statusCode === '0000') {
        _this.setState({
                      ...
                      <form>
                        <div className='form-group mb-0'>
                          <div className='input-group input-group-lg input-group-flush'>
                            ...
                            <input type='search' className='form-control' placeholder={t`lanCommonLabelSearch`} value={this.state.searchString} onKeyPress={this.handleOnKeyPress} onChange={(search) => this.handleSearch(search)} />
                          </div>
                        </div>
                      </form>
                    </div>
