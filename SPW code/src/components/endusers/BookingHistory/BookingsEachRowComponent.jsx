  ...
  handleBookingCancelAmount (item) {
    ...
    if (item.paymentStatus === 'Pending') {
      ...
      if (hours < item.spPropertyInfoId.pricing.refundCancelTime) {
        ...
        let appCharges = Math.ceil((item.totalPrice) * (appPer / 100))
        if (hours >= item.spPropertyInfoId.pricing.fullRefundCancelTime) {
          let refundAmount = Math.floor((item.totalPrice) - appCharges)
          this.setState({
            modalIsOpen: true,
          ...
          let refundAmount = Math.floor(((item.totalPrice) - (appCharges)) * (item.spPropertyInfoId.pricing.refundCancelPercentage / 100))
          this.setState({
            modalIsOpen: true,
          ...
          ToastsStore.error(t`lanEULabelErrorSomethingWentWrong`)
        }
      }
    }
  }
  handleBookingCancel () {
    ...
    APICallManager.putCall(obj, function (resObj) {
      if (resObj.data.statusCode === '0000') {
        ...
        ToastsStore.error(t`lanEULabelErrorCancellationFailedTryAgain`)
        _this.setState({ confirmCancelModalVisible: false, bookingCode: '', checkInDate: '', spServiceProviderId: '' })
      }
    })
  }
              ...
              return (
                <tbody key={i}>
                  <tr>
                    ...
                    <td>
                      <span className='text-muted'>{item.bookingType === 'Hours' ? `${item.totalDays} day(s) / ${item.totalHours} hour(s)` : `${item.totalDays} day(s) `} </span>
                    </td>
          ...
          <div className='container modalOne'>
            <div className='row my-2'>
              <div className='col-sm-12 text-right'>
                <i className='fas fa-times' onClick={this.closeModaltwo} />
              </div>
              <div className='col-sm-12 m-3'>
                <p>{t`lanEULabelAreYouSureYouWantToCancelThisBooking`}<br />{t`lanEULabelOn`} {moment(this.state.checkInDate).format('MMM DD, YY')} {t`lanEULabelTo`}
        ...
        >
          <div className='container modalOne'>
            <div className='row my-2'>
              ...
              <div className='col-sm-12 m-3'>
                {!this.state.noRefund
              ? <p>{this.state.refundAmount} {t`lanEULabelWillBeRefundedTo`} {this.state.euName} <br /> {t`lanEULabelWithBookingId`} {this.state.bookingCode}</p>
              : <p>{this.state.refundAmount}<br /> {t`lanEULabelWithBookingId`} {this.state.bookingCode}</p>
              }
              </div>
            </div>
            <div className='row my-3'>
