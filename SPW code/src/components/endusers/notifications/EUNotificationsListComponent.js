...
class EUNotificationsListComponent extends React.Component {
  ...
  componentWillMount () {
    let obj = {
      unBlockStatus: true,
      url: config.baseUrl + config.getEUNotificationsAPI }
    let _this = this
  ...
  handleDelete (item) {
    ...
    APICallManager.putCall(delobj, function (delResObj) {
      ...
      if (delResObj.data.statusCode === '0000') {
        ...
        ToastsStore.error(t`lanEULabelNotificationsDeleteFailed`)
      }
    })
  }
  handleHome (event) {
                      ...
                      <div className='col-md-12 col-lg-12 col-xl-12'>
                        ...
                        ? this.state.notificationData.map((item, i) =>
                          <div className='row align-items-center bottom-line py-1' key={i}>
                            <div className='col-sm-11 col-11'>
                              <a onClick={() => this.handleRedirect(item)}>
                                <div className='row align-items-center py-1 list-group-item-action'>
                                  <div className='col-sm-1 img-mobile col-2'>
                                    {/* Avatar */}
                                  ...
                                  <div className='col-10 col-sm-9'>
                                    ...
                                    <p className={item.status === 'Unread' ? 'text-sm mb-0 pr-3 font-weight-bold lineHt' : 'text-sm mb-0 pr-3 lineHt'}>{t`lanEULabelBookingCode`}
                                      : <span style={{ color: 'green' }} className='font-weight-500'> {item.notificationBody.bookingCode}</span></p>
                                    <p className={item.status === 'Unread' ? 'text-sm mb-0 pr-3 font-weight-bold' : 'text-sm mb-0 pr-3'}>{t`lanEULabelContact`}: <u>{item.notificationBody.spContact}</u></p>
                                  </div>
                                  <div className='col-sm-2 notify-date-mobile'>
                            ...
                            <div className='col-sm-1 col-1 delete-col-m'>
