...
export default class EUFavouritesListComponent extends React.Component {
  ...
  handlePageChange (pageNumber) {
    ...
    let favouritesList = {
      unBlockStatus: true,
      url: config.baseUrl + config.getEUFavouritesListAPI + pageNumber + '/' + this.state.searchString
    }
    let _this = this
    APICallManager.getCall(favouritesList, function (resObj) {
      if (resObj.data.statusCode === '0000') {
                            ...
                            <form>
                              <div className='form-group mb-0'>
                                <div className='input-group input-group-lg input-group-flush'>
                                  ...
                                  <input
                                    type='search' className='form-control' value={this.state.searchString} onKeyPress={this.handleOnKeyPress} onChange={this.handleInputChange} placeholder={t`lanCommonLabelSearch`} />
                                </div>
                              </div>
                            </form>
                          </div>
