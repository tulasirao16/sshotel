            ...
            <div className='room'>
              <figure>
                <a className='avatar rounded-circle avatar-sm fav-icon' title='liked' style={{ backgroundColor: '#ffffff' }} >
                  {this.state.isFavourite ? <i className='fas fa-heart' disabled={this.state.submitDisabled} onClick={() => this.handleUnFavouriteProperty(data)} /> : <i className='far fa-heart' disabled={this.state.submitDisabled} onClick={() => this.handleFavouriteProperty(data)} /> }
                </a>
                <a>
              ...
              <div style={{ cursor:'pointer' }} role='button' disabled={this.state.submitDisabled} onClick={() => this.handleBooking(data)}>
                <div className='caption'>
                  ...
                  {data.spPropertyId.rating ? <div className='txt2 pb-0'>
                    {/* {data.rating} */}
                    <div className='small-stars pl-0'>
                      <StarRatingComponent
                        name='rate1'
                        starCount={data.spPropertyId.rating}
                        value={data.spPropertyId.rating}
                      />
                    </div>
                  </div>
                  ...
                  <div className='txt4'>
                    <a className='eu-font'>{t`lanEUButtonViewDetail`}</a>
                  </div>
                </div>
                <div className='select-txt'>
                  <a style={{ textDecoration: null }}><span className='eu-font'>{t`lanEUButtonSelectThisRoom`}<i className='fa fa-angle-right' aria-hidden='true' /></span></a>
                </div>
              </div>
            </div>
          </div>
