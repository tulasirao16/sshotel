...
class SPCreateHourlyBookingComponent extends React.Component {
  constructor (props) {
    super(props)
  ...
  getNumberOfRoomsCount (checkInDate, checkOutDate, checkInTime, checkOutTime) {
    ...
    let obj = { url: config.baseUrl + config.postEUBookingRoomsCountAPI, routing: '/host/bookings-history', body: body }
    let _this = this
  ...
  handleRooms (value, membersCapacity, childsCapacity) {
    ...
    if (value === 'add') {
      ...
      this.setState({ numOfRooms: addRoom, errorMessage: '' })
    } else {
      let subRoom = (this.state.numOfRooms) - (1)
      if (subRoom < roomsCount || subRoom < totalRoomsCount) {
      } else {
        let subRoom = (this.state.numOfRooms) - (1)
        this.setState({ numOfRooms: subRoom, errorMessage: '' })
      }
    }
  }
  handleMobileGetDetails () {
    ...
    if (!this.state.mobileNumber) {
      this.setState({ errorMessage: t`lanSPLabelErrorPleaseProvideMobileNumber` })
    } else if (reg.test(this.state.mobileNumber) === false) {
      this.setState({ errorMessage: t`lanSPLabelErrorInvalidNumberPleaseProvideAValidMobileNumber` })
    } else {
      let getEUDetails = {
        unBlockStatus: true,
  ...
  handleEmailGetDetails () {
    ...
    if (checkText === true) {
      ...
      if (this.state.email === '' || this.state.email === 'undefined') {
        this.setState({ errorMessage: t`lanSPLabelErrorPleaseEnterAEmail` })
      } else if (reg.test(this.state.email) === false) {
        this.setState({ errorMessage: t`lanSPLabelErrorPleaseEnterAValidEmail` })
      } else {
        let getEUDetails = {
          unBlockStatus: true,
  ...
  handleCreateBooking () {
    ...
    if (!this.state.checkInDate) {
      ...
      let _this = this
      let obj = { url: config.baseUrl + config.postSPEndUserBookingAPI, routing: '/host/bookings-history', body: postJson }
      APICallManager.postCall(obj, function (resObj) {
        if (resObj.data.statusCode === '0000') {
          ...
          ToastsStore.error(t`lanSPLabelErrorServiceIsNotAvailableOnSelectedDates`)
        ...
        <div className='row'>
          <div className='col-md-6'>
            <div className='row'>
              <div className='col-md-6'>
                <div className='form-group'>
                  <label className='form-control-label'>{t`lanSPLabelNoOfDaysHours`} (#)<span className='error'>*</span></label>
                  {this.state.days === 0
                    ? <p className='price-sq form-control mb-0'> {`${this.state.hours} hours`}</p>
                    : this.state.days >= 2
                    ? <p className='price-sq form-control mb-0'> {`${this.state.days} days / ${this.state.hours} hours` }</p>
                    : <p className='price-sq form-control mb-0'> {`${this.state.days} day / ${this.state.hours} hours` }</p> }
                </div>
              </div>
              <div className='col-md-6'>
                <div className='form-group'>
                  <label className='form-control-label'>{t`lanSPLabelTotal`} {t`lanCommonLabelBookingAmount`} (₹)<span className='error'>*</span></label>
                  <input type='text' value={(this.state.numOfRooms) * (this.state.amount)} className='form-control' id='ticketOn' readOnly />
                  <span className='input-error-icon' ><i className='far fa-times-circle' style={this.state.ticketOnError ? { color: 'red', fontSize: 13 } : { opacity: 0 }} /></span>
                  <p className='text-muted'><small style={{ color: 'red', fontSize: 13 }}>{this.state.ticketOnError}</small> </p>
                </div>
              </div>
          ...
          <div className='col-md-6'>
            ...
            {(this.state.numOfAdults) > ((this.state.avaliableRoomCount) * (this.state.propertyInfoData.membersCapacity) - 1) || ((this.state.numOfAdults) + (this.state.numOfChilds) > ((this.state.avaliableRoomCount) * (parseInt(this.state.propertyInfoData.membersCapacity) + parseInt(this.state.propertyInfoData.childsCapacity)) - 1))
            ? <label>{'Reached Maximum Limit Of Adults'}</label>
            : ((this.state.numOfChilds) > (this.state.avaliableRoomCount) * (this.state.propertyInfoData.childsCapacity) - 1) && ((this.state.numOfAdults) > (this.state.avaliableRoomCount) * (this.state.propertyInfoData.membersCapacity) - 1) || ((this.state.numOfAdults) + (this.state.numOfChilds) > ((this.state.avaliableRoomCount) * (parseInt(this.state.propertyInfoData.membersCapacity) + parseInt(this.state.propertyInfoData.childsCapacity)) - 1))
              ? <label>{'Reached Maximum Limit Of Childs'}</label>
              : this.state.avaliableRoomCount <= this.state.numOfRooms
              ? <label>{'Reached Maximum Limit Of Rooms'}</label>
            : null
            }
          </div>
        </div>
...
export default SPCreateHourlyBookingComponent
