...
class CreateBookingPropertiesListComponent extends React.Component {
  ...
  componentWillMount () {
    // let propertiesList = { url: config.baseUrl + config.getSPCreateBookingPropertiesDataAPI + 'Days' + '/1/' }
    // let _this = this
    // APICallManager.getCall(propertiesList, function (resObj) {
    //   if (resObj && resObj.data.statusCode === '0000') {
    //     _this.setState({ propertiesList: resObj.data.statusResult.homeData, totalCount: resObj.data.statusResult.totalDocs })
    //   } else {
    //     _this.setState({ propertiesList: [], totalCount: 0 })
    //   }
    // })
    let obj = {
      unBlockStatus: true,
      url: config.baseUrl + config.getSPPropertiesListAPI + this.state.activePage + '/' }
    let _this = this
  ...
  handlePageChangePropertiesList (pageNumber) {
    ...
    // let SPBookingType = !this.state.hostsBy === true ? 'Hours' : 'Days'
    // let propertiesList = { url: config.baseUrl + config.getSPCreateBookingPropertiesDataAPI + SPBookingType + '/' + pageNumber + '/' + this.state.searchString }
    // let _this = this
    // APICallManager.getCall(propertiesList, function (resObj) {
    //   if (resObj && resObj.data.statusCode === '0000') {
    //     _this.setState({ propertiesList: resObj.data.statusResult.homeData, totalCount: resObj.data.statusResult.totalDocs })
    ...
    // })
      ...
      let obj = {
        unBlockStatus: true,
        url: config.baseUrl + config.getSPPropertiesListAPI + pageNumber + '/' + this.state.searchString }
      let _this = this
      APICallManager.getCall(obj, function (resObj) {
        if (resObj.data.statusCode === '0000') {
          _this.setState({
  ...
  //   let propertiesList = { url: config.baseUrl + config.getSPCreateBookingPropertiesDataAPI + this.state.hostsBy + '/' + '1' + '/' + event.target.value }
  //   APICallManager.getCall(propertiesList, function (resObj) {
  //     if (resObj && resObj.data.statusCode === '0000') {
  //       _this.setState({ propertiesList: resObj.data.statusResult.homeData, totalCount: resObj.data.statusResult.totalDocs })
  //     } else {
  ...
  handleHostsBy () {
    ...
    let SPBookingType = !this.state.hostsBy === true ? 'Hours' : 'Days'
    localStorage.setItem('SPBookingType', JSON.stringify(SPBookingType))
    let propertiesList = {
      unBlockStatus: true,
      url: config.baseUrl + config.getSPCreateBookingPropertiesDataAPI + SPBookingType + '/1/' }
    let _this = this
    APICallManager.getCall(propertiesList, function (resObj) {
      if (resObj && resObj.data.statusCode === '0000') {
        _this.setState({ propertiesList: resObj.data.statusResult.homeData, totalCount: resObj.data.statusResult.totalDocs })
          ...
          <div className='card property-list'>
            ...
            <div className='card-body'>
              {/* <div className='row justify-content-center bg-light py-1'>
                ...
                <p className='pt-2'>Hourly Hotels</p>
              </div> */}
                        ...
                        <div className='row'>
                          <div className='col-md-4 left_div'>
                            <div className='card-images p-0'>
                              <img src={item.propertyId && item.propertyId.imagePath ? config.baseUrl + item.propertyId.imagePath : require('../images/room1.jpg')} className='respnsive-img' />
                            </div>
                          </div>
