...
class SPNotificationsListComponent extends React.Component {
  ...
  componentWillMount () {
    let obj = {
      unBlockStatus: true,
      url: config.baseUrl + config.getSPNotificationsListAPI + this.state.activePage + '/' }
    let _this = this
  ...
  handlePageChange (pageNumber) {
    if (this.state.activePage !== pageNumber) {
      ...
      let obj = {
        unBlockStatus: true,
        url: config.baseUrl + config.getSPNotificationsListAPI + pageNumber + '/' + this.state.searchString }
      APICallManager.getCall(obj, function (resObj) {
        if (resObj.data.statusCode === '0000') {
  ...
  handleRedirect (item) {
    if (item.notificationBody && item.notificationBody.type === 'Booking' && item.notificationBody.recordId) {
      ...
      let obj = {
        unBlockStatus: true,
        url: config.baseUrl + config.getSPPropertyDataAPI + item.notificationBody.recordId }
      APICallManager.getCall(obj, function (resObj) {
        if (resObj.data.statusCode === '0000') {
      ...
      let obj = {
        unBlockStatus: true,
        url: config.baseUrl + config.getSPUserDataAPI + item.notificationBody.recordId }
      APICallManager.getCall(obj, function (resObj) {
        if (resObj.data.statusCode === '0000') {
                ...
                <div className='col-lg-8 col-7'>
                  ...
                  <nav aria-label='breadcrumb' className='d-none d-md-inline-block ml-md-4'>
                    <ol className='breadcrumb breadcrumb-links breadcrumb-dark'>
                      ...
                      {this.state.isNotificationView ? <li className='breadcrumb-item active' aria-current='page'>{t`lanSPTitleNotificationDelete`}</li> : ''}
                    </ol>
                  </nav>
                </div>
              </div>
                                ...
                                <a className='list-group-item list-group-item-action' onClick={() => this.handleRedirect(item)}>
                                  <div className='row align-items-center'>
                                    <div className='col-12'>
                                      ...
                                      <p className={item.status === 'Unread' ? 'text-sm mb-0 pr-3 font-weight-bold' : 'text-sm mb-0 pr-3'}>{item.notificationMessage}</p>
                                      {item.notificationBody.bookingCode
                                      ? <p className={item.status === 'Unread' ? 'text-sm mb-0 pr-3 font-weight-bold' : 'text-sm mb-0 pr-3'}>Booking Code: {item.notificationBody.bookingCode}</p>
                                      : null }
                                      <p className={item.status === 'Unread' ? 'text-sm mb-0 pr-3 font-weight-bold' : 'text-sm mb-0 pr-3'}>Contact: {item.notificationBody.contact}</p></div>
                                  </div>
                                </a>
