...
class SPBlockedDatesEditComponent extends React.Component {
  constructor () {
    super()
    this.state = {
  ...
  handleUpdateBlockedDates () {
    if (!this.state.blockingType) {
      this.setState({ errorMessage: t`lanSPLabelErrorBlockingTypeIsRequired` })
     ...
     moment(this.state.to).format('YYYY-MM-DD') === moment(this.state.dummyToDate).format('YYYY-MM-DD')) {
      ...
      let obj = { url: config.baseUrl + config.putSPPropertyBlockedDatesEditAPI + this.state.blockedObj._id, body: postData }

      let _this = this
      APICallManager.putCall(obj, function (resObj) {
        _this.setState({ blockingType: 'Continuous Blocking', from: '', to: '' })
        if (resObj.data.statusCode === '0000') {
          ToastsStore.success(t`lanSPLabelSuccessBlockedDatesUpdatedSuccessfully`)
          setTimeout(() => {
            hashHistory.push('/host/property-view')
          }, 2000)
          // hashHistory.push('/host/property-view')
              ...
              <div className='card mb-2'>
                ...
                <div className='card-body'>
                  <div className='form-group row'>
                    ...
                    <div className='col-sm-8 mt-4'>
                      {this.state.blockingType === 'Continuous Blocking'
                        ? <div className='row'>
                          ...
                          {t`lanSPLabelBlockDatesSelected`} {' '} {t`lanSPLabelBlockDatesFrom`} <span className='error mx-3 block-from'>
                            {from &&
                              to &&
                    ...
                    <div className='col-md-12 text-center'>
                      <button disabled={this.state.buttonDisabled} className='btn btn-primary update-edit' onClick={this.handleUpdateBlockedDates}>{t`lanCommonButtonUpdate`}</button>
...
SPBlockedDatesEditComponent.propTypes = {
  blockedObj: PropTypes.any
}

export default SPBlockedDatesEditComponent
