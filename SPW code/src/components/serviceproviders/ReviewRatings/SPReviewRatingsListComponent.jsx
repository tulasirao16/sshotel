...
class SPReviewRatingsListComponent extends React.Component {
  ...
  componentWillMount () {
    ...
    if (isTimeBased === 'True') {
      obj = {
        unBlockStatus: true,
        url: config.baseUrl + config.getSPReviewRatingsListAPI + byDateType + '/' + this.state.activePage + '/' }
      this.setState({ isTimeBased: true })
    } else {
      obj = {
        unBlockStatus: true,
        url: config.baseUrl + config.getSPReviewRatingsListAPI + 'All' + '/' + this.state.activePage + '/' }
  ...
  handleInputChange (event) {
    ...
    if (isTimeBased === 'True') {
      obj = {
        unBlockStatus: true,
        url: config.baseUrl + config.getSPReviewRatingsListAPI + byDateType + '/' + this.state.activePage + '/' + event.target.value }
      this.setState({ isTimeBased: true })
    } else {
      obj = {
        unBlockStatus: true,
        url: config.baseUrl + config.getSPReviewRatingsListAPI + 'All' + '/' + this.state.activePage + '/' + event.target.value }
      this.setState({ isTimeBased: false })
    }
    // let obj = {
    //   unBlockStatus: true,
  ...
  handlePageChangeReviewRatingList (pageNumber) {
    if (this.state.activePage !== pageNumber) {
      ...
      if (isTimeBased === 'True') {
        obj = {
          unBlockStatus: true,
          url: config.baseUrl + config.getSPReviewRatingsListAPI + byDateType + '/' + pageNumber + '/' }
        this.setState({ isTimeBased: true })
      } else {
        obj = {
          unBlockStatus: true,
          url: config.baseUrl + config.getSPReviewRatingsListAPI + 'All' + '/' + pageNumber + '/' }
        this.setState({ isTimeBased: false })
      }
      // let obj = {
      //   unBlockStatus: true,
      //   url: config.baseUrl + config.getSPReviewRatingsListAPI + this.state.isTimeBased + '/' + pageNumber + '/' + this.state.searchString }
      let _this = this
      APICallManager.getCall(obj, function (resObj) {
        if (resObj.data.statusCode === '0000') {
          _this.setState({
