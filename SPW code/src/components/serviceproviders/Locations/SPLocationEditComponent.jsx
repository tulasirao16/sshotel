    ...
    if (!this.state.address.trim()) {
      ...
      APICallManager.putCall(obj, function (resObj) {
        ...
        if (resObj.data.statusCode === '0000') {
          ToastsStore.success(t`lanSPLabelSuccessLocationUpdatedSuccessfully`)
          setTimeout(() => {
            hashHistory.push('/host/locations')
          }, 2000)
        } else {
          ToastsStore.error(t`lanSPLabelErrorLocationUpdateFailed`)
        }
      })
    }
  }
              ...
              <div className='card border-0 mb-0'>
                ...
                <div className='card-body px-lg-4 py-lg-4'>
                  <form role='form'>
                    ...
                    <div>
                      <small><span style={{ color: 'red' }}>{ t`lanSPLabelNote`} :</span> <b>{ t`lanSPLabelIfNotMappedTypeTheAddressManually`}</b></small>
                      <div className='text-center'>
                        <button type='button' className='btn btn-primary mt-2' disabled={this.state.submitDisabled} onClick={this.handleUpdateLocationDetails}>{ t`lanCommonButtonUpdate` }</button>
                      </div>
                    </div>
                  </form>
                </div>
