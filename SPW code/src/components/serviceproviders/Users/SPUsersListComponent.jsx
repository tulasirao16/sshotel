...
class SPUsersListComponent extends React.Component {
  ...
  handlePageChange (pageNumber) {
    if (this.state.activePage !== pageNumber) {
      ...
      let usersList = {
        unBlockStatus: true,
        url: config.baseUrl + config.getSPUsersListAPI + '/' + pageNumber + '/' + this.state.searchString }
      let _this = this
      APICallManager.getCall(usersList, function (resObj) {
        if (resObj.data.statusCode === '0000') {
          _this.setState({
  ...
  handleConfirmDeleteUser () {
    ...
    let _this = this
    let obj = { url: config.baseUrl + config.postSPUserDeleteAPI, routing: 'host/users', body: userInfo }
                              ...
                              <form>
                                <div className='form-group mb-0'>
                                  <div className='input-group input-group-lg input-group-flush'>
                                    ...
                                    <input type='search' className='form-control' value={this.state.searchString} onChange={this.handleInputChange} onKeyPress={this.handleOnKeyPress} placeholder={t`lanCommonLabelSearch`} />
                                  </div>
                                </div>
                              </form>
                            </div>
