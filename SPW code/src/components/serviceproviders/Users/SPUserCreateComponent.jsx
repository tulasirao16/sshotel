...
class SPUserCreateComponent extends React.Component {
  constructor () {
    super()
    this.state = {
      firstName: '',
  ...
  handleCreateUser () {
    ...
    if (!this.state.firstName.trim()) {
      ...
      let obj = { url: config.baseUrl + config.postSPUserCreateAPI, routing: 'host/users', body: userData }
      APICallManager.postCall(obj, function (resObj) {
        _this.setState({ buttonDisabled: true })
        if (resObj.data.statusCode === '0000') {
          ToastsStore.success(t`lanSPLabelSuccessUserCreatedSuccessfully`)
          setTimeout(() => {
            _this.props.handleUserCreatedData(resObj.data.statusResult)
          }, 2000)
        } else {
          ToastsStore.error(t`lanSPLabelErrorUserCreateFailed`)
    ...
    return (
      <div className='user-create'>
        <div className='py-lg-4'>
          <div className='mb-3'><h3>{ t`lanSPButtonCreateUser` }</h3></div>
          <form>
            <div className='row'>
          ...
          <form>
            <div className='row'>
              ...
              <div className='col-md-3 pr-1'>
                <div className='form-group'>
                  ...
                  <input type='number' className='form-control' name='zip' maxLength='6' value={this.state.zip} onChange={() => this.setState({ zip: event.target.value, errorMessage:'' })} />
                </div>
              </div>
            </div>
            <div className='row'>
            ...
            <div className='row'>
              <div className='update ml-auto mr-auto'>
                <button type='submit' disabled={this.state.buttonDisabled} className='btn btn-primary btn-round' onClick={this.handleCreateUser}>{ t`lanCommonButtonCreate` }</button>
              </div>
            </div>
          </form>
...
export default SPUserCreateComponent
