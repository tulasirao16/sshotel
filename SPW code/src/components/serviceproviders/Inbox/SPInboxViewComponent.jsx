  ...
  handleConfirmDeleteMessage () {
    ...
    APICallManager.postCall(obj, function (resObj) {
      ...
      if (resObj.data.statusCode === '0000') {
        ...
        _this.setState({ errorMessage: 'lanSPLabelErrorMessageDeleteFailed' })
      }
    })
  }
                  ...
                  <div className='col-lg-6 col-7'>
                    ...
                    <nav aria-label='breadcrumb' className='d-none d-md-inline-block ml-md-4'>
                      <ol className='breadcrumb breadcrumb-links breadcrumb-dark'>
                        ...
                        <li className='breadcrumb-item'><a onClick={() => this.props.handleInboxView(this.state.messageStatus, this.state.messagesData)} >{ t`lanSPLabelInbox` }</a></li>
                        <li className='breadcrumb-item active' aria-current='page'>{ t`lanSPLabelReadMessage` }</li>
                      </ol>
                    </nav>
                  </div>
                </div>
              ...
              <div className='col-md-3'>
                <div className='list-group list-group-messages'>
                  <a className='list-group-item active border-0'>
                    <i className='far fa-envelope mr-1' /> { t`lanSPLabelInbox` }
                <span className='badge badge-dark badge-pill float-right'>8</span>
                  </a>
                  <a className='list-group-item list-group-item-action border-0'>
                    <i className='fa fa-paper-plane-o mr-1' /> { t`lanSPLabelSent` }</a>
                  <a className='list-group-item list-group-item-action border-0'>
                    <i className='far fa-file mr-1' /> { t`lanSPLabelDraft` }</a>
                  <a className='list-group-item list-group-item-action border-0'>
                    <i className='far fa-star mr-1' /> { t`lanSPLabelStared` }
                      <span className='badge badge-danger badge-pill float-right'>3</span>
                  </a>
                  <a className='list-group-item list-group-item-action border-0'>
                    <i className='fas fa-trash-alt mr-2' /> { t`lanSPLabelTrash` }
                    </a>
                </div>
              </div>
                ...
                <div className='card mb-2'>
                  ...
                  <div className='card-footer'>
                    <div className='pull-right'>
                      <button type='button' className='btn' onClick={() => this.handleReplay(this.state.messagesData)}><i className='fas fa-reply' /> {t`lanSPButtonReply`}</button>
                      {/* <button type='button' className='btn'><i className='fa fa-share' /> Forward</button> */}
                    </div>
                    <button type='button' className='btn' onClick={() => this.handleDelete(this.state.messagesData)}><i className='fa fa-trash-o' /> {t`lanSPButtonDelete`}</button>
