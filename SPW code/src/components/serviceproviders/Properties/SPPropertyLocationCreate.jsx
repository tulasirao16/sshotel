...
class SPPropertyLocationCreate extends React.Component {
  constructor (props) {
    ...
    let alternateMobileNumber = authObj && authObj.alternateMobileNumber ? authObj.alternateMobileNumber : ''
        ...
        this.getLocation(position.coords.latitude, position.coords.longitude, function (data) {
          if (data.statusCode === '0000') {
            ...
            data.result.forEach(value => {
              ...
              //   _this.setState({ address: _this.state.houseNumber ? _this.state.houseNumber + ', ' : '' + _this.state.lineName ? _this.state.lineName + ', ' : '' + _this.state.street ? _this.state.street + ', ' : '' + value.long_name })
              // }
            ...
            ToastsStore.error(t`lanSPLabelErrorNotGettingLocationPleaseEnterManually`)
          }
        })
      },
    ...
    if (event.target.value === 'Select Locations List') {
      ...
      if (authObj && authObj.userAccount) {
        this.setState({
          ...
          alternateMobileNumber: authObj && authObj.alternateMobileNumber ? authObj.alternateMobileNumber : '',
          email: authObj.email,
  ...
  handleGetLatLong = () => {
    ...
    let _this = this
    fetch(config.googleGeoLocationAddress + addressLocation + ',IN&key=' + config.googleMapsAPIKey)
    .then((response) => response.json())
    .then((responseJson) => {
      _this.setState({
  ...
  handleSubmitLocationDetails () {
    ...
    if (!this.state.address.trim()) {
      ...
      this.setState({ errorMessage: t`lanSPLabelErrorCountryIsRequired` })
    } else if (!this.state.latitude) {
      this.setState({ errorMessage: t`lanSPLabelErrorLatitudeIsRequired` })
    } else if (!this.state.longitude) {
      this.setState({ errorMessage: t`lanSPLabelErrorLongitudeIsRequired` })
      ...
      ToastsStore.success(t`lanSPSuccessPropertyLocationCreated`)
    ...
    return (
      <div>
        <form role='form'>
          ...
          <div>
            <small><span style={{ color: 'red' }}>{t`lanSPLabelNote`} :</span> <b>{t`lanSPMessageIfNotMapped`}</b></small>
