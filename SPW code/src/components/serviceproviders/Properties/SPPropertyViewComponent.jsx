...
import SPPropertyLocationEdit from './SPPropertyLocationEdit'
import SPPropertyNearestAreasUpdate from './SPPropertyNearestAreasUpdate'
import SPBlockedDatesList from '../../../containers/serviceproviders/BlockedDates/SPBlockedDatesList'
import SPPropertyInfoList from '../../../containers/serviceproviders/PropertyInfos/SPPropertyInfoList'
import SPPropertyImagesEdit from './SPPropertyImagesEdit'
...
class SPPropertyViewComponent extends React.Component {
  constructor () {
    super()
    this.state = {
      propertyObj: {},
  ...
  componentWillMount () {
    ...
    if (propertyData && propertyData._id) {
      this.setState({
        ...
        propertyTitleDummy: propertyData.propertyTitle,
        aboutPropertyDummy: propertyData.aboutProperty,
        propertyTypeDummy: propertyData.propertyType,
      ...
      let obj = {
        unBlockStatus: true,
        url: config.baseUrl + config.getSPPropertyDocsAPI + propertyData._id }
      let _this = this
  ...
  onFileChange = (e) => {
    ...
    if ((previousImagesLength + fileslength + fdLength) <= 10) {
      ...
      this.setState({ videoErrorMessage: t`lanSPLabelErrorUploadUpToTenImagesOnly` })
    }
  }
  onFileChangeVideo = (e) => {
    this.setState({ videoError: false })
    if (this.state.videoOld && this.state.videoOld.length > 0) {
      this.setState({ videoErrorMessage: t`lanSPLabelErrorToUploadNewVideoPleaseDeleteExistingVideo` })
    } else {
      let _this = this
      ...
      reader.onloadend = function (e) {
        if (file.size > 150000000) {
          this.setState({ videoErrorMessage: t`lanSPLabelErrorUploadedVideoSizeShouldBeLessThanOrEqualTo250MBAndDurationShouldBeLessThanOrEqualTo60Seconds`, videoError: true })
        } else {
          let media = new Audio(reader.result)
          media.onloadedmetadata = function () {
            if (media.duration > 60) {
              _this.setState({ videoErrorMessage: t`lanSPLabelErrorUploadedVideoDurationShouldBeLessThanOrEqualTo60SecondsAndSizeShouldBeLessThanOrEqualTo250MB`, videoError: true })
            }
          }
        }
        this.setState({
  ...
  handlePropertyUpdate () {
    if (this.state.images.length < 1) {
      ...
      this.setState({ errorMessage: t`lanSPLabelErrorUploadUpToTenImagesOnly` })
    } else if (this.state.videoError) {
      this.setState({ errorMessage: t`lanSPLabelErrorUploadedVideoDurationShouldBeLessThanOrEqualTo60SecondsAndSizeShouldBeLessThanOrEqualTo250MB` })
      ...
      async.series([
        ...
        fetchPolyfill(config.baseUrl + config.putSPPropertyUpdateAPI + _this.state.propertyId, {
          method: 'PUT',
          body: data,
          headers: {
          ...
          response.json().then((responseJson) => {
            if (responseJson.statusCode === '0000') {
              ...
              ToastsStore.success(t`lanSPSuccessPropertyUpdatesuccessfully`)
              setTimeout(() => {
                hashHistory.push('/host/properties')
  ...
  imageFunction = (images, files, video, videoFiles, videoError) => {
    ...
    // let video = []
    // photos.forEach(item => {
    //   if (item && item.imagePath && item.imagePath.indexOf('.mp4') === -1) {
    //     images.push(item)
    //   } else if (item && item.imagePath && item.imagePath.indexOf('.mp4') !== -1) {
    //     video.push(item)
    //   } else if (item && item.indexOf('image') !== -1) {
    //     images.push(item)
    //   } else {
    //     video.push(item)
    //   }
    // })
    // this.setState({ propertyDocs: photos, images: images, video: video, photosStatus: false })
    if (!videoError) {
          ...
          <div className='card property-view'>
                ...
                <div className='col-md-4 text-right'>
                  {(this.state.images && this.state.images.length > 0) || (this.state.video && this.state.video.length > 0) && (!this.state.photosStatus)
                    ? <div className='custom-file mb-2 col-md-6 text-right'>
                      <button className='btn btn-info' onClick={() => this.setState({ photosStatus: true })}>{t`lanSPButtonViewAllImages`}</button>
                    </div>
                    : null }
                </div>
              </div>
            ...
            <div className='card-body'>
              {this.state.photosStatus
               ? <SPPropertyImagesEdit photos={this.state.images} propertyImagesCount={this.state.propertyImagesCount} video={this.state.video} newFiles={this.state.newFiles} imageFunction={this.imageFunction}
                 propertyID={this.state.propertyId} imagePath={this.state.imagePath} videoFiles={this.state.videoFiles} videoError={this.state.videoError} />
              : <div>
                <div className='row'>
                  <div className='col-sm-6 create-prop-slider image-col'>
                    ...
                    <div className='add-image-btn'>
                      ...
                      <label className='btn btn-primary add-icon'><i className='fas fa-plus' /><br />{t`lanSPLabelPhotos`}</label>
                    </div>
                  </div>
                  <div className='col-md-6 video-col'>
                    ...
                    <div className='row mt-4'>
                      <div className='add-image-btn'>
                        ...
                        <label className='btn btn-primary add-icon'><i className='fas fa-plus' /><br /> {t`lanSPLabelVideo`}</label>
                      </div>
                    </div>
                  </div>
                      ...
                      <div className='accordion' id='accordionExample'>
                        ...
                        <div className='card'>
                          ...
                          <div id='collapseTwo' className='collapse' aria-labelledby='headingTwo' data-parent='#accordionExample'>
                            <div className='card-body'>
                              <SPPropertyLocationEdit spLocationObj={this.state.spLocationData} spLocationId={this.state.spLocationId} propertyId={this.state.propertyId} />
                            </div>
                          </div>
                        </div>
                        <div className='card'>
                          ...
                          <div id='collapseThree' className='collapse' aria-labelledby='headingThree' data-parent='#accordionExample'>
                            <div className='card-body'>
                              <SPPropertyNearestAreasUpdate spLocationObj={this.state.spLocationData} nearestAreas={this.state.nearestAreas} commonFunction={this.commonFunction} />
                            </div>
                          </div>
                        </div>
                        <div className='card'>
                          ...
                          <div id='collapseFour' className='collapse' aria-labelledby='headingFour' data-parent='#accordionExample'>
                            <div className='card-body'>
                              <SPBlockedDatesList propertyID={this.state.propertyId} propertyObj={this.state.propertyObj} />
                            </div>
                          </div>
                        </div>
                        <div className='card'>
                          ...
                          <div id='collapseFive' className='collapse' aria-labelledby='headingFive' data-parent='#accordionExample'>
                            <div className='card-body'>
                              ...
                              <SPPropertyInfoList propertyID={this.state.propertyId} propertyObj={this.state.propertyObj} />
                            </div>
                          </div>
                        </div>
                      </div>
                ...
                <div className='row'>
                  <div className='col-sm-12 text-center mt-4'>
                    <button className='btn btn-danger' type='button' onClick={this.handleBack}>{t`lanCommonButtonBack`}</button>
                    <button disabled={this.state.buttonDisabled} className='btn btn-primary' type='button' onClick={this.handlePropertyUpdate}>{t`lanCommonButtonUpdate`}</button>
                  </div>
                </div>
              </div>}
            </div>
            ...
            >
              <div>
                <p>{t`lanSPLabelYouHaveSelected`}{this.state.imagesChoosedLength}{t`lanSPLabelImages`}</p>
...
export default SPPropertyViewComponent
