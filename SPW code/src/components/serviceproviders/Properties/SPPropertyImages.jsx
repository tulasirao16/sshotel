...
export default class SPPropertyImages extends React.Component {
  ...
  handleBack = () => {
    this.props.imageFunction(this.state.photos, this.state.files, this.state.video, this.state.videoSelectNDel ? true : this.state.videoErro)
  }
  handleSelect (i) {
  ...
  handleDelete = () => {
    ...
    async.series([
      ...
      _this.props.imageFunction(imageData, newFiles, video, videoFiles, _this.state.videoError)
    })
  }
  render () {
    return (
      <div className='view-property-files'>
        <div className='row'>
          {this.state.photos && this.state.photos.length > 0 ? this.state.photos.map((item, i) => {
            return (
              <SPPropertyImageView image={item} keyPos={i} handleSelect={this.handleSelect} selected={false} key={i} />
            )
          })
          ...
          {this.state.video && this.state.video.length > 0 ? this.state.video.map((item, i) => {
            return (
              <div className='col-md-3 view-all-images' key={i}>
...
class SPPropertyImageView extends React.Component {
  ...
  render () {
    return (
      <div className='col-md-3 view-all-images' key={this.state.keyPos} >
