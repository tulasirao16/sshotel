...
class SPProfileIdProofEditComponent extends React.Component {
  constructor () {
    super()
    this.state = {
  ...
  handleIdTypeValidation (event) {
    ...
    APICallManager.getCall(userObj, function (resObj) {
      if (resObj.data.statusCode === '0000') {
        _this.setState({ idTypeValid: false, errorMessage: t`lanSPLabelErrorYouHaveThisIdTypeInYourList` })
      } else {
  ...
  handleUpdateIdProof () {
    ...
    if (!this.state.idTypeValid) {
      this.setState({ errorMessage: t`lanSPLabelErrorYouHaveThisIdTypeInYourList` })
  ...
  handleApiCall () {
    ...
    fetchPolyfill(config.baseUrl + config.putSPUserProfileIdProofUpdateAPI + this.state._id, {
      method: 'PUT',
      body: data,
      ...
      response.json().then((resData) => {
        if (resData.statusCode === '0000') {
          ToastsStore.success(t`lanSPLabelErrorIDProofUpdatedSuccessfully`)
          setTimeout(() => {
            hashHistory.push('/host/user/profile/idproof')
          }, 2000)
          ...
          ToastsStore.error(t`lanSPLabelErrorIDProofUpdatedFailed`)
        }
      })
    })
  }
                  ...
                  <form>
                        ...
                        <div className='form-group'>
                          ...
                          <select className='form-control' disabled value={(this.state.idProofData && this.state.idProofData.idType) ? this.state.idProofData.idType : ''} onChange={this.handleIdTypeValidation} >
                            {(this.state.idProofData && this.state.idProofData.idType)
                              ? null : <option value=''>{ t`lanSPLabelPleaseSelectIdType` }</option>}
                    ...
                    <div className='row'>
                      <div className='col-sm-12 text-center '>
                        <button type='button' className='btn btn-primary' disabled={this.state.submitDisabled} onClick={this.handleUpdateIdProof}>{ t`lanCommonButtonUpdate` }</button>
                      </div>
                    </div>
                  </form>
                </div>
        ...
        <Modal ariaHideApp={false} isOpen={this.state.modalIsOpen} style={customStyles}>
          <div className='modal-content'>
            <div className='modal-header p-3' style={{ backgroundColor:'#e4e4e4' }}>
              <h4 className='modal-title'>{ t`lanSPTitleIDProofView` }</h4>
              <button type='button' className='close' onClick={this.closeModal}>&times;</button>
            </div>
            <div className='modal-body'>
              <div className='row'>
                <div className='col-md-6'>
                  <label>{ t`lanCommonLabelIDType` }</label>
            ...
            <div className='modal-footer p-3' style={{ backgroundColor:'#e4e4e4' }}>
              <button className='btn btn-primary' onClick={this.handleOk}>{ t`lanSPButtonOK` }</button>
...
export default SPProfileIdProofEditComponent
