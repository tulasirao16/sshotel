...
class SPProfilePreferencesComponent extends React.Component {
  constructor () {
    super()
    this.state = {
  ...
  handleUpdate () {
    ...
    let obj = { url: config.baseUrl + config.putSPProfilePreferencesUpdateAPI + '_id', body: putData }
    APICallManager.putCall(obj, function (resObj) {
      _this.setState({ submitDisabled: true })
      if (resObj.data.statusCode === '0000') {
        ToastsStore.success(t`lanSPLabelSuccessUpdatedSuccessfully`)
        setTimeout(() => {
          hashHistory.push('/host/user/profile')
        }, 2000)
      } else {
        ToastsStore.error(t`lanSPLabelErrorUpdateFailed`)
      }
    })
  }
                  ...
                  <form>
                    ...
                    <div className='row'>
                      <div className='col-sm-12 text-center mt-4'>
                        <button type='button' className='btn btn-primary' disabled={this.state.submitDisabled} onClick={this.handleUpdate}>{ t`lanCommonButtonUpdate` }</button>
...
export default SPProfilePreferencesComponent
