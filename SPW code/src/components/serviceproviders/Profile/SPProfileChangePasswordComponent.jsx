...
class SPProfileChangePasswordComponent extends React.Component {
  constructor () {
    super()
    this.state = {
      currentPassword: '',
  ...
  handleUpdatePassword () {
    if (!this.state.currentPassword) {
      ...
      let obj = { url: config.baseUrl + config.putSPProfileChangePasswordAPI, body: updateData }
      APICallManager.putCall(obj, function (res) {
        if (res.data.statusCode === '1012') {
          localStorage.clear()
          ToastsStore.success(t`lanSPSuccessPasswordUpdatedSuccessfully`)
          setTimeout(() => {
            hashHistory.push('/host/signin')
          }, 2000)
        } else if (res.data.statusCode === '9979') {
          ToastsStore.error(t`lanSPLabelErrorInvalidCurrentPassword`)
        } else {
          ToastsStore.error(t`lanSPLabelErrorPasswordUpdateFailed`)
        }
      })
    }
  }
                  ...
                  <form>
                    ...
                    <div className='row'>
                      <div className='col-sm-12 text-center'>
                        <button type='button' className='btn btn-primary' onClick={this.handleUpdatePassword}>{ t`lanCommonButtonUpdate` }</button>
...
export default SPProfileChangePasswordComponent
