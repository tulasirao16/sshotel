...
class SPPropertyInfoListComponent extends React.Component {
  ...
  componentWillMount () {
    ...
    let obj = {
      unBlockStatus: true,
      url: config.baseUrl + config.getSPPropertyInfosListAPI + this.props.propertyID }
    let _this = this
  ...
  handleStatusChange (status, item, i) {
    ...
    APICallManager.putCall(obj, function (resObj) {
      if (resObj.data.statusCode === '0000') {
        ...
        _this.setState({ reload: true })
        ToastsStore.error(t`lanSPLabelErrorPropertyInfoInactive`)
      } else {
        _this.setState({ reload: true })
                ...
                {this.state.propertyInfoList && this.state.propertyInfoList.length > 0 ? this.state.propertyInfoList.map((item, i) =>
                  <div className='col-md-6' key={i}>
                    <div className='card mb-3'>
                      <a onClick={() => this.handlePropertyInfoView(item, i)}>
                        <div className='row'>
