...
class SPBookingsListComponent extends React.Component {
  ...
  handleSearch = (event) => {
    ...
    let getSpBookingsObj = {
      unBlockStatus: true,
      url: config.baseUrl + config.getSPBookingsHistoryListAPI + 1 + '/' + this.state.status + '/' + event.target.value
    }
    APICallManager.getCall(getSpBookingsObj, function (resObj) {
      if (resObj.data.statusCode === '0000') {
        _this.setState({
  ...
  handleTabs = (key) => {
    this.setState({ key: key, activePage : 1 })
    let _this = this
    switch (key) {
      case '0' :
        let getSpBookingsObj = {
          unBlockStatus: true,
          url: config.baseUrl + config.getSPBookingsHistoryListAPI + 1 + '/' + 'all' + '/' + this.state.searchString
        }
        APICallManager.getCall(getSpBookingsObj, function (resObj) {
          if (resObj.data.statusCode === '0000') {
            _this.setState({
      ...
      case '1' :
        let getSpBookedBookingsObj = {
          unBlockStatus: true,
          url: config.baseUrl + config.getSPBookingsHistoryListAPI + 1 + '/' + 'Booked' + '/' + this.state.searchString
        }
      ...
      case '2' :
        let getSpCompletedBookingsObj = {
          unBlockStatus: true,
          url: config.baseUrl + config.getSPBookingsHistoryListAPI + 1 + '/' + 'Completed' + '/' + this.state.searchString
        }
  ...
  handleBookingCancel (data) {
    ...
    APICallManager.getCall(getSpBookingsObj, function (resObj) {
      if (resObj.data.statusCode === '0000') {
        ...
        _this.setState({
          refundAmount: t`lanSPLabelErrorNoteNoRefundOnCancellation`, modalIsOpen: !_this.state.modalIsOpen, statusCode: resObj.data.statusCode, noRefund: false
        })
      } else {
        _this.setState({
          refundAmount: t`lanSPLabelAreyouSureWantToCancelThisBooking`, modalIsOpen: !_this.state.modalIsOpen, statusCode: resObj.data.statusCode, noRefund: false
        })
      }
    })
  }
  ...
  handleConfirmCancel (statusCode) {
    ...
    switch (statusCode) {
      case '0000' :
        ...
        APICallManager.putCall(obj, function (resObj) {
          if (resObj.data.statusCode === '0000') {
            ...
            ToastsStore.success(t`lanSPSuccessBookingCancellationSuccessful`)
          } else {
            ToastsStore.warning(t`lanSPErrorBookingCancellationFailed`)
          }
        })
        break
      case '8888' :
        ...
        let noRefundObj = { url: config.baseUrl + config.putSPConfirmCancelBookingAPI, body: cancelNoRefundObj }
      ...
      case '7777' :
        ...
        let paymentStatusPendingCancel = { url: config.baseUrl + config.putSPConfirmCancelBookingAPI, body: paymentStatusPendingCancelObj }
        APICallManager.putCall(paymentStatusPendingCancel, function (resObj) {
          if (resObj.data.statusCode === '0000') {
            ...
            ToastsStore.warn(t`lanSPErrorBookingCancellationFailed`)
          }
        })
        break
      default :
        ToastsStore.warning(t`lanSPErrorPleaseTryAgain`)
    }
  }
  closeConfirmModal () {
                            ...
                            <div className='form-group mb-0'>
                              <div className='input-group input-group-lg input-group-flush'>
                                ...
                                <input type='search' className='form-control' placeholder={t`lanCommonLabelSearch`} value={this.state.searchString}
                                  onKeyPress={this.handleOnKeyPress} onChange={(search) => this.handleSearch(search)} />
                              </div>
                            </div>
                          </form>
                        </div>
                      ...
                      <div className='col-md-12 col-lg-12 col-xl-12'>
                                      ...
                                      <tbody key={i}>
                                        <tr>
                                          ...
                                          <td className='table-actions btn-actions'>
                                            ...
                                            {item.bookingStatus === 'Closed' || item.bookingStatus === 'Completed' || item.bookingStatus === 'Cancelled'
                                            ? ''
                                ...
                                { this.state.bookingDate.map((item, i) =>
                                  <tbody key={i}>
                                    <tr>
                                      ...
                                      {this.state.statusChecked ? <td className='text-muted'>{item.bookingStatus}</td> : null }
                                      <td className='table-actions btn-actions'>
                                        <a onClick={() => this.handleBookingView(item)} className='table-action table-action-view' data-toggle='tooltip' data-placement='top' title={t`lanCommonButtonTooltipViewBooking`}>
                                          <i className='far fa-eye' />
                                        </a>
                                        <a onClick={() => this.handleBookingEdit(item)} className='table-action table-action-edit' data-toggle='tooltip' data-placement='top' title={t`lanCommonButtonTooltipEditBooking`}>
                                          <i className='far fa-edit' />
                                        </a>
                                        {item.bookingStatus === 'Booked' || item.bookingStatus === 'Confirmed'
                                    ...
                                    <tbody key={i}>
                                      <tr>
                                        ...
                                        <td className='table-actions btn-actions'>
                                          ...
                                          <a onClick={() => this.handleBookingEdit(item)} className='table-action table-action-edit'
                        ...
                        >
                         ...
                         ? <p>{t`lanSPLabelTotalAmountOf`} {this.state.refundAmount} {t`lanSPLabelWillBeRefundedToTheCustomer`}
                           {this.state.selectedBooking.euName} With Booking ID: {this.state.selectedBooking.bookingCode}</p>
                          : <p>{this.state.refundAmount} {t`lanSPLabelWithBookingID`} {this.state.selectedBooking.bookingCode}</p> }
