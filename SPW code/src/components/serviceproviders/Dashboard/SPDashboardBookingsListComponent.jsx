...
class SPDashboardBookingsListComponent extends React.Component {
  constructor () {
    super()
    this.state = {
      bookingData: [],
      ...
      selectedBooking : {},
      byDateType: localStorage.getItem('byDateType')
    }
  ...
  componentWillMount () {
    this.setState({ bookingCodeChecked:this.state.bookingCodeChecked === 'unchecked',
      emailChecked:this.state.emailChecked === 'unchecked',
      propertyTitleChecked:this.state.propertyTitleChecked === 'unchecked'
    })
    let type = localStorage.getItem('homeBookingType')
    let byDateType = localStorage.getItem('byDateType')
    let getSpBookingsObj = {}
    switch (type) {
      case 'CheckIn':
        getSpBookingsObj = {
          url: config.baseUrl + config.getSPHomeCheckInBookingsListAPI + byDateType + '/' + this.state.activePage + '/' + this.state.searchString
        }
        break
      case 'CheckOut':
        getSpBookingsObj = {
          url: config.baseUrl + config.getSPHomeCheckOutBookingsListAPI + byDateType + '/' + this.state.activePage + '/' + this.state.searchString
        }
        break
      case 'Bookings':
        this.setState({ heading: 'Bookings' })
        getSpBookingsObj = {
          url: config.baseUrl + config.getSPHomeTodayBookingsListAPI + byDateType + '/' + this.state.activePage + '/' + this.state.searchString
        }
        break
      case 'Cancelled':
        this.setState({ heading: 'Cancelled Bookings' })
        getSpBookingsObj = {
          url: config.baseUrl + config.getSPHomeCancelledBookingsListAPI + byDateType + '/' + this.state.activePage + '/' + this.state.searchString
        }
        break
      case 'Amounts':
        this.setState({ heading: 'Booking Amounts' })
        getSpBookingsObj = {
          url: config.baseUrl + config.getSPHomeAmountsBookingsListAPI + byDateType + '/' + this.state.activePage + '/' + this.state.searchString
        }
        break
    }
  ...
  handleInputChange (event) {
    ...
    switch (type) {
      case 'CheckIn':
        getSpBookingsObj = {
          url: config.baseUrl + config.getSPHomeCheckInBookingsListAPI + this.state.byDateType + '/' + '1' + '/' + event.target.value
        }
        break
      case 'CheckOut':
        getSpBookingsObj = {
          url: config.baseUrl + config.getSPHomeCheckOutBookingsListAPI + this.state.byDateType + '/' + '1' + '/' + event.target.value
        }
        break
      case 'Bookings':
        this.setState({ heading: 'Bookings' })
        getSpBookingsObj = {
          url: config.baseUrl + config.getSPHomeTodayBookingsListAPI + this.state.byDateType + '/' + '1' + '/' + event.target.value
        }
        break
      case 'Cancelled':
        this.setState({ heading: 'Cancelled Bookings' })
        getSpBookingsObj = {
          url: config.baseUrl + config.getSPHomeCancelledBookingsListAPI + this.state.byDateType + '/' + '1' + '/' + event.target.value
        }
        break
      case 'Amounts':
        this.setState({ heading: 'Booking Amounts' })
        getSpBookingsObj = {
          url: config.baseUrl + config.getSPHomeAmountsBookingsListAPI + this.state.byDateType + '/' + '1' + '/' + event.target.value
        }
        break
    }
  ...
  handlePageChange (pageNumber) {
    if (this.state.activePage !== pageNumber) {
      ...
      switch (type) {
        case 'CheckIn':
          getSpBookingsObj = {
            url: config.baseUrl + config.getSPHomeCheckInBookingsListAPI + this.state.byDateType + '/' + pageNumber + '/' + this.state.searchString
          }
          break
        case 'CheckOut':
          getSpBookingsObj = {
            url: config.baseUrl + config.getSPHomeCheckOutBookingsListAPI + this.state.byDateType + '/' + pageNumber + '/' + this.state.searchString
          }
          break
        case 'Bookings':
          this.setState({ heading: 'Bookings' })
          getSpBookingsObj = {
            url: config.baseUrl + config.getSPHomeTodayBookingsListAPI + this.state.byDateType + '/' + pageNumber + '/' + this.state.searchString
          }
          break
        case 'Cancelled':
          this.setState({ heading: 'Cancelled Bookings' })
          getSpBookingsObj = {
            url: config.baseUrl + config.getSPHomeCancelledBookingsListAPI + this.state.byDateType + '/' + pageNumber + '/' + this.state.searchString
          }
          break
        case 'Amounts':
          this.setState({ heading: 'Booking Amounts' })
          getSpBookingsObj = {
            url: config.baseUrl + config.getSPHomeAmountsBookingsListAPI + this.state.byDateType + '/' + pageNumber + '/' + this.state.searchString
          }
          break
      }
  ...
  handleBookingView (data) {
    localStorage.setItem('BookingViewFrom', 'Bookings Page')
    localStorage.setItem('bookingData', JSON.stringify(data))
    hashHistory.push('/host/bookings-history/view')
    event.preventDefault()
  }
  handleBookingEdit (data) {
    localStorage.setItem('BookingEditFrom', 'Bookings Page')
    localStorage.setItem('bookingData', JSON.stringify(data))
  ...
  handleBookingCancel (data) {
    ...
    APICallManager.getCall(getSpBookingsObj, function (resObj) {
      if (resObj.data.statusCode === '0000') {
        _this.setState({
          refundAmount: resObj.data.statusResult.result.totalRefundAmount, modalIsOpen: !_this.state.modalIsOpen, confirmCancelBookingData: resObj.data.statusResult.result
        })
      } else {
        ToastsStore.error(t`lanSPErrorAlertSorryYouCantCancelThisBooking`)
      }
    })
  }
  closeModal () {
  ...
  handleConfirmCancel () {
    ...
    APICallManager.putCall(obj, function (resObj) {
      if (resObj.data.statusCode === '0000') {
        ...
        alert(t`lanSPErrorAlertBookingCancellationFailed`)
      }
    })
  }
  closeConfirmModal () {
                ...
                <div className='col-lg-6 col-7'>
                  ...
                  <nav aria-label='breadcrumb' className='d-none d-md-inline-block ml-md-4'>
                    <ol className='breadcrumb breadcrumb-links breadcrumb-dark'>
                      ...
                      <li className='breadcrumb-item active' aria-current='page'>{ t`lanSPButtonBookingsPage` }</li>
                    </ol>
                  </nav>
                </div>
              </div>
                            ...
                            <div className='form-group mb-0'>
                              <div className='input-group input-group-lg input-group-flush'>
                                ...
                                <input type='search' className='form-control' placeholder={t`lanCommonLabelSearch`} value={this.state.searchString}
                                  onKeyPress={this.handleOnKeyPress} onChange={(search) => this.handleInputChange(search)} />
                              </div>
                            </div>
                          </form>
                        </div>
                ...
                <div className='card-body bookings'>
                  <section className='notifications'>
                    ...
                    <Modal
                      ...
                      ariaHideApp={false}
                    >
                      <p>{t`lanSPLabelTotalAmountOf`} {this.state.refundAmount} {t`lanSPLabelWillBeRefundedToTheCustomer`}
                        {this.state.selectedBooking.euName} {t`lanSPLabelWithBookingID`} {this.state.selectedBooking.bookingCode}</p>
                    ...
                    <Modal
                      ...
                      ariaHideApp={false}
                    >
                      <p>{t`lanSPLabelAreYouSureYouWantToCancelThisBookingOn`} {moment(this.state.selectedBooking.checkInDate).format('MMM DD, YY')} -
...
export default SPDashboardBookingsListComponent
