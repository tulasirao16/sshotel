...
class SPDashboardBlockedDatesListComponent extends React.Component {
  ...
  componentWillMount () {
    ...
    let obj = {
      unBlockStatus: true,
      url: config.baseUrl + config.getSPHomeBlockedDatesAPI + byDateType + '/' + 1 + '/' }
    let _this = this
  ...
  handleInputChange (event) {
    ...
    let usersList = {
      unBlockStatus: true,
      url: config.baseUrl + config.getSPHomeBlockedDatesAPI + byDateType + '/' + this.state.activePage + '/' + event.target.value
    }
  ...
  handlePageChange (pageNumber) {
    if (this.state.activePage !== pageNumber) {
      ...
      let usersList = {
        unBlockStatus: true,
        url: config.baseUrl + config.getSPHomeBlockedDatesAPI + pageNumber + '/' + this.state.searchString }
      let _this = this
      APICallManager.getCall(usersList, function (resObj) {
        if (resObj.data.statusCode === '0000') {
          _this.setState({ blockedDatesList: resObj.data.statusResult.blockedDatesData, totalCount: resObj.data.statusResult.totalDocs })
                ...
                <div className='col-lg-6 col-7'>
                  ...
                  <nav aria-label='breadcrumb' className='d-none d-md-inline-block ml-md-4'>
                    <ol className='breadcrumb breadcrumb-links breadcrumb-dark'>
                      ...
                      <li className='breadcrumb-item active' aria-current='page'>{t`lanSPButtonBlockedDates`}</li>
                    </ol>
                  </nav>
                </div>
              </div>
                        ...
                        <table className='table align-items-center table-flush table-striped'>
                          ...
                          {this.state.blockedDatesList && this.state.blockedDatesList.length > 0 ? this.state.blockedDatesList.map((item, i) =>
                            <tbody key={i}>
                              <tr>
                                <td>
                                  <span className='text-muted'>{item.propertyId.propertyTitle}</span>
                                </td>
                                <td>
