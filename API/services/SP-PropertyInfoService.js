...
module.exports = {
  ...
  getSpPropertyInfos: function (propertyID, callback) {
    SP_PropertyInfoDAO.getSpPropertyInfos(propertyID, function (resObj) {
  ...
  getPropertyInfoPricingList: function (tokendecodeddata, searchString, callback) {
    SP_PropertyInfoDAO.getPropertyInfoPricingList(tokendecodeddata, searchString, function (error, resObj) {
      if (error) {
  ...
  getPropertyInfoGuestRulesList: function (propertyID, propertyInfoId, tokendecodedData, callback) {
    SP_PropertyInfoDAO.getPropertyInfoGuestRulesList(propertyID, propertyInfoId, tokendecodedData, function (resObj) {
      callback({ httpCode: resObj.httpCode, statusCode: resObj.statusCode, result: resObj.result });
    });
  },
  ...
  getPropertyInfoPrice: function (propertyID, propertyInfoId, tokendecodedData, callback) {
    SP_PropertyInfoDAO.getPropertyInfoPrice(propertyID, propertyInfoId, tokendecodedData, function (resObj) {
      callback({ httpCode: resObj.httpCode, statusCode: resObj.statusCode, result: resObj.result });
    });
  },
  ...
  getPropertyInfoServices: function (propertyID, propertyInfoId, tokendecodedData, callback) {
    SP_PropertyInfoDAO.getPropertyInfoServices(propertyID, propertyInfoId, tokendecodedData, function (resObj) {
      callback({ httpCode: resObj.httpCode, statusCode: resObj.statusCode, result: resObj.result });
    });
  },
  ...
  createPropertyInfo: function(reqObj, tokendecodedData, callback) {
    var currentUTC = CommonService.currentUTCObj();
    calculation(reqObj, function (calculationData) {
      var propertyInfoObj = setPropertyInfoData(reqObj, calculationData, tokendecodedData, currentUTC);
      var InfoObj = new SP_PropertyInfo(propertyInfoObj);
      SP_PropertyInfoDAO.createPropertyInfo(InfoObj, tokendecodedData, function (resObj) {
  ...
  updatePropertyInfo: function(reqObj, propertyId, propertyInfoId, tokendecodedData, callback) {
    var currentUTC = CommonService.currentUTCObj();
    ...
    SP_PropertyInfoDAO.updatePropertyInfo(propertyInfoObj, query, reqObj.counts, tokendecodedData, function(resObj) {
      callback({httpCode: resObj.httpCode, statusCode: resObj.statusCode, result: resObj.result});
    });
  },

  updatePropertyInfoPricing: function(propertyInfoId, propertyId, reqObj, tokendecodedData, callback) {
    var currentUTC = CommonService.currentUTCObj();
    calculation(reqObj, function(calculationData) {
      var upPrObj = setPropertyInfoPricing(reqObj, calculationData, currentUTC);
      SP_PropertyInfoDAO.updatePropertyInfoPricing(propertyInfoId, propertyId, upPrObj, tokendecodedData, function(resObj) {
        callback({httpCode: resObj.httpCode, statusCode: resObj.statusCode, result: resObj.result});
      });
    });
  },
  spPropertyInfoStatusChange: function(infoID, status, tokendecodedData, callback) {
    var currentUTC = CommonService.currentUTCObj();
    if(status == 'Inactive') {
    var query = {
      'spServiceProviderId': tokendecodedData.uspid,
    ...
    SP_PropertyInfoDAO.isBookingsAvailable(query, function(resObj) {
      if(resObj.statusCode == '9997') {
        ...
        SP_PropertyInfoDAO.spPropertyInfoStatusChange(infoID, tokendecodedData, updateObj, function(resObj) {
          callback({httpCode: resObj.httpCode, statusCode: resObj.statusCode, result: resObj.result});
        })
      } else if(resObj.statusCode == '0000') {
  ...
  updateAmenitiesInPropertyInfo: function(propertyInfoId, reqBody, tokendecodedData, callback) {
    var currentUTC = CommonService.currentUTCObj();
    var updateObj = {};
    if(reqBody.type == 'amenities') {
    updateObj = {
      amenities: reqBody.avaliableData,
      ...
      updateObj = {
        services: reqBody.avaliableData,
        updatedBy: tokendecodedData.ua,
        updatedAt: currentUTC.currentUTCDateTimeNumber,
        updatedOn: currentUTC.currentUTCDateTimeString
      };
    }
    SP_PropertyInfoDAO.updateAmenitiesInPropertyInfo(propertyInfoId, tokendecodedData, updateObj, function(resObj) {
      callback({httpCode: resObj.httpCode, statusCode: resObj.statusCode, result: resObj.result});
    });
  }
}
...
var recursiveGuestRules = function(i, resObj, guestRules, tokenDecodedData, callback) {
  if( i < guestRules.length) {
      ...
      SP_PropertyInfoDAO.setGuestRuleData(ruleGuestObj, tokenDecodedData, function(reResObj) {
...
var recursiveInfoAmenities = function(i, resObj, amenities, tokendecodedData, callback) {
  if( i < amenities.length) {
    ...
    SP_PropertyInfoDAO.setInfoAmenityData(amtyObj, tokendecodedData, function(reResObj) {
...
var recursiveInfoPaidServices = function(i, resObj, paidServices, tokendecodedData, callback) {
  if( i < paidServices.length) {
    ...
    SP_PropertyInfoDAO.setInfoPaidServicesData(serviceObj, tokendecodedData, function(reResObj) {
...
function calculation(reqObj, callback) {
  ...
  async.series([
    ...
    function(callback) {
      pgChargesHrs = {
        // pgChargesHrs = (discountHrsPrice + cgstHrsPrice + sgstHrsPrice + serviceCharges + othercharges + appChargesHrs) * appPgPercentage/100;
       ...
       // with out Discount Pg Charges
        // pgChargesHrs = (hrsPrice + cgstHrsPrice + sgstHrsPrice + serviceCharges + othercharges appChargesHrs) * appPgPercentage/100;
    ...
    function(callback) {
      spAmountHrs = {
        //UIside// spAmountHrs = totalHrsPrice - pgChargesHrs - appChargesHrs + serviceCharges + othercharges
        // With Discount 
...
function setPropertyInfoData (reqObj, calculationData, tokendecodedData, currentUTC) {
  var infoObj = {
      spServiceProviderId: tokendecodedData.uspid,
